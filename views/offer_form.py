...
class OfferFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        subject_row.grid(row=3, column=0, columnspan=2, sticky="ew", pady=(8, 0))
        self._add_row_field(subject_row, "Titulli / Subjekti i Ofertës:", "subject_entry", 0)
        
        # Font Size Setting
        font_settings = ctk.CTkFrame(info_grid, fg_color="transparent")
        font_settings.grid(row=3, column=2, sticky="ew", pady=(8, 0), padx=(10, 0))
        ctk.CTkLabel(font_settings, text="Madhësia e Tekstit:", font=ctk.CTkFont(family="Inter", size=11, weight="bold"), text_color=self.text_main).pack(anchor="w")
        self.font_size_combo = ctk.CTkComboBox(font_settings, values=["Auto", "10", "11", "12", "13", "14", "15", "16"], height=28, font=ctk.CTkFont(size=12), fg_color="#F8F9FA", button_color="#F8F9FA")
        self.font_size_combo.pack(fill="x")
        self.font_size_combo.set("Auto")

        fields_row.columnconfigure((0,1), weight=1)
        subject_row.columnconfigure(0, weight=1)
        info_grid.columnconfigure(0, weight=5)
        ...
        self.btn_preview = ctk.CTkButton(btn_box, text="👁 Shiko", command=self.process_preview, fg_color="#F1F3F5", hover_color="#E9ECEF", text_color=self.text_main, font=ctk.CTkFont(family="Inter", size=13, weight="bold"), width=90, height=38, corner_radius=8, cursor="hand2")
    ...
    def add_aligned_row(self, initial_data=None, row_type='item'):
        ...
        if row_type == 'header':
            ...
            # Compact height for single-line look
            height = 35 if row_type == 'item' else 60
            placeholder = "Përshkrimi..." if row_type == 'item' else "Tekst informativ..."
            desc_text = ctk.CTkTextbox(row_frame, height=height, font=ctk.CTkFont(size=12), fg_color="#FBFBFB", border_width=1, border_color="#E9ECEF", corner_radius=6)
            desc_text.grid(row=0, column=1, sticky="ew", padx=(10, 5), pady=10)
            
            if row_type == 'item':
                # Calculation Container to the RIGHT of description
                calc_bar = ctk.CTkFrame(row_frame, fg_color="transparent", height=40)
                calc_bar.grid(row=0, column=2, sticky="ew", padx=(5, 10))
                
                # Horizontal Modules List
                modules_list_frame = ctk.CTkFrame(calc_bar, fg_color="transparent")
                modules_list_frame.pack(side="left", fill="x")
                
                subtotal_label = ctk.CTkLabel(calc_bar, text="", width=0) # Logic only
                
                # Add Module Button
                add_mod_btn = ctk.CTkButton(calc_bar, text="+", width=24, height=24, font=ctk.CTkFont(size=14, weight="bold"), 
                                            fg_color="transparent", text_color=self.primary_color, hover_color="#FFF4ED",
                                            border_width=1, border_color=self.primary_color, corner_radius=6, 
                                            command=lambda: self.add_module_to_row(row_id))
                add_mod_btn.pack(side="left", padx=5)
        ...
        # Border Checkbox for ALL row types
        self.border_var = ctk.BooleanVar(value=initial_data.get('has_border', True) if (initial_data and isinstance(initial_data, dict)) else True)
        border_chk = ctk.CTkCheckBox(row_frame, text="Kornizë", variable=self.border_var, width=60, height=20, font=ctk.CTkFont(size=11), checkbox_width=18, checkbox_height=18, text_color=self.text_secondary)
        border_chk.grid(row=0, column=3, padx=(5, 5))
        
        # Delete Row Button (Far Right)
        ...
        del_row_btn.grid(row=0, column=4, sticky="e", padx=5)
        ...
        if row_type == 'item' and not initial_data:
            ...
            self.add_module_to_row(row_id, "Sasia", "m²")
    ...
    def add_module_to_row(self, row_id, label_text="", unit_text=""):
        ...
        if len(item['modules']) > 0:
            ctk.CTkLabel(mod_frame, text="x", font=ctk.CTkFont(size=13, weight="bold"), text_color="#6C757D").pack(side="left", padx=10)

        # Styled Input Group (Chip Style)
        input_container = ctk.CTkFrame(mod_frame, fg_color="#F1F3F5", corner_radius=8, border_width=1, border_color="#DEE2E6")
        input_container.pack(side="left", pady=2)
        
        # Value
        val_entry = ctk.CTkEntry(input_container, width=60, height=28, font=ctk.CTkFont(family="Inter", size=12), fg_color="#F1F3F5", border_width=0, placeholder_text="0", text_color="#1A1A1A")
        val_entry.pack(side="left", padx=(5, 0))
        
        # Unit (ComboBox for quick selection + custom typing)
        unit_values = ["€", "m²", "m'", "m³", "copë"]
        unit_entry = ctk.CTkComboBox(input_container, width=85, height=28, font=ctk.CTkFont(family="Inter", size=11, weight="bold"), 
                                     values=unit_values,
                                     fg_color="#F1F3F5", border_width=0, button_color="#F1F3F5",
                                     button_hover_color="#E9ECEF", dropdown_font=ctk.CTkFont(size=11),
            ...
            if t == 'item':
                ...
                if custom_attr:
                    if isinstance(custom_attr, dict):
                        # New structure: {'modules': [], 'has_border': True}
                        modules_list = custom_attr.get('modules', [])
                        has_border = custom_attr.get('has_border', True)
                    elif isinstance(custom_attr, list):
                        # Old structure: List of modules
                        modules_list = custom_attr
                
                # Set border state
                if last_item['border_var']:
                    last_item['border_var'].set(has_border)
                
                if modules_list:
                    for attr in modules_list:
    ...
    def get_offer_data(self):
        ...
        for item_entry in self.items:
            ...
            # Retrieve border state
            has_border = item_entry['border_var'].get() if item_entry.get('border_var') else False

            # Wrap into new dictionary structure
            ...
            items.append({
                ...
                'unit_price': price,
                'row_type': t,
                'custom_attributes': custom_attributes_payload
            })
    ...
    def process_save(self, action='save'):
        ...
        if offer.save():
            ...
            try:
                if action == 'pdf':
                    manual_fs = self.font_size_combo.get()
                    generator = PDFGenerator()
                    output_path = generator.generate_offer(offer, manual_font_size=manual_fs)
    ...
    def process_preview(self):
        """Generates a temporary PDF for previewing without saving to DB"""
        offer = self.get_offer_data()
        if not offer: return
        
        # Temporary ID for generation logic (if required)
        if not offer.id: offer.id = 999999 
        
        orig_text = self.btn_preview.cget("text")
        self.btn_preview.configure(text="Duke u hapur...", state="disabled"); self.update_idletasks()
        
        try:
            import tempfile
            manual_fs = self.font_size_combo.get()
            temp_path = os.path.join(tempfile.gettempdir(), f"Preview_Oferta_{datetime.now().strftime('%H%M%S')}.pdf")
            
            generator = PDFGenerator()
            generator.generate_offer(offer, output_path=temp_path, manual_font_size=manual_fs)
            
            if os.path.exists(temp_path):
                os.startfile(temp_path)
            else:
                messagebox.showerror("Gabim", "Nuk u arrit të gjenerohej PDF për preview.")
        except Exception as e:
            messagebox.showerror("Gabim", f"Gabim gjatë preview: {str(e)}")
        finally:
            self.btn_preview.configure(text=orig_text, state="normal")
