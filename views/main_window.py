...
class MainWindow(ctk.CTk):
    ...
    def create_sidebar(self):
        ...
        self.status_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent", height=20)
        self.status_frame.pack(pady=(0, 10))
        
        self.status_dot = ctk.CTkLabel(self.status_frame, text="●", font=ctk.CTkFont(size=14))
        self.status_dot.pack(side="left", padx=5)
        
        self.status_text = ctk.CTkLabel(self.status_frame, text="DUKE U LIDHUR...", font=ctk.CTkFont(size=10, weight="bold"))
        ...
        nav_info = [
            ...
            ("Lista ofertave", "📂", self.show_offer_list),
    ...
    def update_status_indicator(self):
        """Përditëson indikatorin e lidhjes pa bllokuar UI"""
        import threading
        ...
        def do_check():
            try:
                # Ky proces zgjat deri ne 3 sekonda nese s'ka internet
                Database.check_connection_status()
                if self.winfo_exists():
                    self.after(0, lambda: self._apply_status(Database._is_offline))
            except: pass
            finally:
                self._checking_status = False
                # Kontrolli i radhës pas 5 sekondave për bateri/perf, 
                # por me 2s nëse sapo ka nisur
                wait_time = 5000 if Database._cloud_conn else 2000
                if self.winfo_exists():
                    self.after(wait_time, self.update_status_indicator)

        thread = threading.Thread(target=do_check, daemon=True)
        thread.start()

    def _apply_status(self, is_offline):
        if not self.winfo_exists(): return
        
        if is_offline:
            self.status_dot.configure(text_color="#E74C3C") # E kuqe
            self.status_text.configure(text="OFFLINE (LOKAL)", text_color="#E74C3C")
        else:
            self.status_dot.configure(text_color="#2ECC71") # Jeshile
            self.status_text.configure(text="ONLINE (CLOUD)", text_color="#2ECC71")
