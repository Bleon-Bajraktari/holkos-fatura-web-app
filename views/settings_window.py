...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        # --- Seksioni: Cilësimet e Aplikacionit ---
        ctk.CTkLabel(form_frame, text="CILËSIMET E APLIKACIONIT", font=ctk.CTkFont(size=14, weight="bold"), text_color="#FF6600").grid(row=9, column=0, columnspan=2, pady=(20, 5), sticky="w")
        
        self.show_status_switch = ctk.CTkSwitch(form_frame, text="Aktivizo Menaxhimin e Statusit (Paguar/Pa Paguar)")
        self.show_status_switch.grid(row=10, column=0, columnspan=2, padx=10, pady=10, sticky="w")
        
        # --- Seksioni: Cilësimet e Email-it (SMTP) ---
        ctk.CTkLabel(form_frame, text="CILËSIMET E EMAIL-IT (SMTP)", font=ctk.CTkFont(size=14, weight="bold"), text_color="#FF6600").grid(row=11, column=0, columnspan=2, pady=(20, 5), sticky="w")
        
        self._add_field(form_frame, "SMTP Server:", "smtp_server_entry", 12)
        self._add_field(form_frame, "SMTP Port:", "smtp_port_entry", 13)
        self._add_field(form_frame, "SMTP Email:", "smtp_user_entry", 14)
        
        # Password me maskim
        ctk.CTkLabel(form_frame, text="SMTP Password:", font=ctk.CTkFont(size=12, weight="bold")).grid(row=15, column=0, padx=10, pady=10, sticky="w")
        self.smtp_password_entry = ctk.CTkEntry(form_frame, width=400, show="*")
        self.smtp_password_entry.grid(row=15, column=1, padx=10, pady=10, sticky="ew")
        
        info_txt = "Për Gmail, përdorni 'App Password'. Porti 587 për TLS."
        ctk.CTkLabel(form_frame, text=info_txt, font=ctk.CTkFont(size=10), text_color="gray").grid(row=16, column=1, sticky="w", padx=10)
    ...
    def load_company_data(self):
        ...
        # Ngarko cilësimet e aplikacionit
        try:
            res = self.db.execute_query("SELECT setting_value FROM settings WHERE setting_key = 'feature_payment_status'")
            val = res[0]['setting_value'] if res else 'true'
            if val == 'true': self.show_status_switch.select()
            else: self.show_status_switch.deselect()
        except: pass
    ...
    def save_settings(self):
        ...
        # Ruaj cilësimet e app
        status_val = 'true' if self.show_status_switch.get() == 1 else 'false'
        try:
            self.db.execute_query("""
                INSERT INTO settings (setting_key, setting_value) VALUES ('feature_payment_status', %s)
