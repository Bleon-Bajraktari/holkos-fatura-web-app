...
const Dashboard = () => {
    ...
    const statCards = [
        { label: 'Totali i Faturave', value: stats?.total_invoices || 0, icon: <FileText size={20} />, color: 'bg-blue-500', trend: `${stats?.total_offers || 0} Oferta` },
        { label: 'Të hyrat Totale', value: `${(stats?.total_revenue || 0).toLocaleString('sq-AL')} €`, icon: <TrendingUp size={20} />, color: 'bg-green-500', trend: stats?.growth > 0 ? `+${stats.growth}% këtë muaj` : `${stats?.growth || 0}% këtë muaj` },
        { label: 'Të hyrat e Muajit', value: `${(stats?.current_month_revenue || 0).toLocaleString('sq-AL')} €`, icon: <Calendar size={20} />, color: 'bg-amber-500', trend: 'Muaji aktual' },
        { label: 'Klientë Aktivë', value: stats?.total_clients || 0, icon: <Users size={20} />, color: 'bg-purple-500', trend: 'Në sistem' },
    ]

    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto w-full space-y-10">
            <header>
                <h1 className="text-3xl font-black text-slate-800 tracking-tight">Mirësevini përsëri</h1>
                <p className="text-slate-400 font-medium mt-1">Ja një përmbledhje e aktivitetit tuaj të biznesit.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
                    ...
                    <div className="space-y-4">
                        {stats?.recent_activity?.map((act: any, idx: number) => (
                            <div key={idx} className="flex items-center justify-between p-4 rounded-3xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100 group">
                ...
                <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-8 rounded-[3rem] text-white shadow-2xl shadow-blue-900/20 flex flex-col justify-between">
                    <div>
