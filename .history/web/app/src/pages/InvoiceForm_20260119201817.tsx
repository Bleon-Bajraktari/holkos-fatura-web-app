...
const InvoiceForm = () => {
    ...
    useEffect(() => {
        ...
        if (isEdit) {
            setLoading(true)
            InvoiceService.getOne(parseInt(id))
                .then(data => {
                    setInvoice({
                        ...data,
            ...
            fetch('/api/invoices/next-number')
                .then(res => res.json())
                .then(data => setInvoice(prev => ({ ...prev, invoice_number: data.next_number })))
        }
    }, [id, isEdit])
    ...
    const handleSave = async (action: 'save' | 'pdf' | 'email') => {
        ...
        try {
            ...
            let savedInvoice;
            if (isEdit) {
                // Not implemented in api.ts yet, but standard
                savedInvoice = await InvoiceService.create(data) // Should be update
            } else {
                savedInvoice = await InvoiceService.create(data)
            }
            ...
            if (action === 'pdf') {
                ...
                // Open email dialog (simplified for now)
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-3xl border border-slate-200/60 shadow-sm">
                        ...
                        <div className="space-y-5">
                            <div>
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">Numri i Faturës</label>
                                <input
                                    type="text"
