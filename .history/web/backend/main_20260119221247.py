...
def get_invoices(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Simple query without joinedload to avoid column alias issues
        ...
        for idx, inv in enumerate(invoices):
            try:
                logger.info(f"Serializing invoice {idx+1}/{len(invoices)}: ID={inv.id}")
                ...
                logger.info(f"✓ Invoice {inv.id} serialized successfully")
                
            except Exception as e:
                logger.error(f"✗ Error serializing invoice {inv.id}: {e}")
                import traceback
                traceback.print_exc()
                continue
        
        logger.info(f"Returning {len(result)} invoices")
        return result
        
    except Exception as e:
        logger.error(f"FATAL ERROR in get_invoices: {e}")
        import traceback
        traceback.print_exc()
