...
def create_invoice(invoice: schemas.InvoiceCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(status_code=400, detail=f"Fatura me numrin '{invoice.invoice_number}' ekziston!")
...
def create_offer(offer: schemas.OfferCreate, db: Session = Depends(get_db)):
    # Check for duplicate
    existing = db.query(models.Offer).filter(models.Offer.offer_number == offer.offer_number).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Oferta me numrin '{offer.offer_number}' ekziston!")
