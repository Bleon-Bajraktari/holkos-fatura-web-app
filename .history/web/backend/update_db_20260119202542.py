...
def update_db():
    ...
    with engine.connect() as conn:
        # Invoices table
        columns = [c['name'] for c in conn.execute(text("SHOW COLUMNS FROM invoices")).fetchall()]
        ...
        if "status" not in columns:
            ...
            conn.execute(text("ALTER TABLE invoices ADD COLUMN status ENUM('draft', 'sent', 'paid') DEFAULT 'draft'"))
        
        # Invoice Items table
        columns = [c['name'] for c in conn.execute(text("SHOW COLUMNS FROM invoice_items")).fetchall()]
