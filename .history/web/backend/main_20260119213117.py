...
def create_invoice(invoice: schemas.InvoiceCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(models.Invoice).filter(
        ...
        extract('year', models.Invoice.date) == invoice.date.year
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Fatura me numrin '{invoice.invoice_number}' ekziston për vitin {invoice.date.year}!")
...
def get_next_offer_number(db: Session = Depends(get_db)):
    ...
    # Get recent offers to find max sequence
    recent_offers = db.query(models.Offer).filter(models.Offer.date >= date(year, 1, 1)).order_by(models.Offer.id.desc()).limit(10).all()
    
    max_seq = 0
    for off in recent_offers:
        try:
            parts = off.offer_number.split('-')
            if len(parts) == 3:
                val = int(parts[2])
                if val > max_seq:
                    max_seq = val
    ...
    next_val = max_seq + 1
    return {"next_number": f"OF-{year}-{next_val:03d}"}
...
def create_offer(offer: schemas.OfferCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(models.Offer).filter(
        ...
        extract('year', models.Offer.date) == offer.date.year
    ).first()
    if existing:
        raise HTTPException(status_code=400, detail=f"Oferta me numrin '{offer.offer_number}' ekziston për vitin {offer.date.year}!")
