...
class Invoice:
    ...
    def get_next_invoice_number(db=None):
        """Gjeneron numrin e ardhshëm të faturave"""
        database = db or Database()
        ...
        if result and len(result) > 0:
            ...
            # Nëse numri është në format "FATURA NR.24", merr numrin
            try:
                if "NR." in last_number.upper():
                    num = int(last_number.split("NR.")[-1].strip())
                    return f"FATURA NR.{num + 1}"
            except:
                pass
    ...
    def save(self):
        ...
        if self.id:
            ...
            params = (self.invoice_number, self.date, self.payment_due_date,
                     ...
                     float(self.total), self.status, self.id)
        else:
            # Insert
            query = """INSERT INTO invoices 
                (invoice_number, date, payment_due_date, client_id, template_id,
                 subtotal, vat_percentage, vat_amount, total, status)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
            params = (self.invoice_number, self.date, self.payment_due_date,
                     self.client_id, self.template_id, float(self.subtotal),
                     float(self.vat_percentage), float(self.vat_amount),
                     float(self.total), self.status)
