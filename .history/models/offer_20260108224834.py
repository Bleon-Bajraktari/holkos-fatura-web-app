...
class Offer:
                ...
                if self.items:
                    ...
                    for idx, item in enumerate(self.items):
                        item_values.append((
                            ...
                            float(item['quantity']), float(item['unit_price']), float(item['subtotal']), idx
                        ))
    ...
    def get_by_id(offer_id, db_connection=None):
        ...
        try:
            ...
            # Get items
            items_result = db.execute_query("SELECT * FROM offer_items WHERE offer_id = %s ORDER BY order_index", (offer_id,))
