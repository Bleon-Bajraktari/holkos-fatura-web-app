...
class Database:
    ...
    def connect(self, force=False):
        """Lidhet me databazat në mënyrë inteligjente dhe të shpejtë"""
        current_time = time.time()
        
        # Throttling: Mos provo shumë shpesh nëse jemi krejtësisht offline
        ...
        # 1. Provo lidhjen Cloud (Primary) - Prioritet
        try:
            # get_connection ka timeout 3s
        ...
        # Nese kemi lidhje Cloud, mos blloko shume per Backup
        try:
            # Ketu nuk perdorim config.get_backup_connection ne kete pike 
            # sepse duam nje timeout shume te shkurter nese jemi vecse Online
            import pymysql
            Database._backup_conn = pymysql.connect(
                ...
                connect_timeout=1, # Vetem 1 sekonde per localhost
                autocommit=True
            )
        except:
