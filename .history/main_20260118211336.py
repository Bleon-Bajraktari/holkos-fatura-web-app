...
def check_database():
    """Kontrollon dhe përditëson strukturën e databazës"""
    # 1. Përditëso/Krijo databazën automatikisht
    try:
        create_database()
    except Exception as e:
        print(f"Paralajmërim gjatë përditësimit të databazës: {e}")

    # 2. Vazhdo me kontrollin normal
    db = Database()
    if not db.connect():
...
def main():
    ...
    try:
        ...
        # SINKRONIZIMI NE BACKGROUND (Local -> Cloud)
        import threading
        from services.sync_service import SyncService
        threading.Thread(target=SyncService.sync_local_to_cloud, daemon=True).start()
