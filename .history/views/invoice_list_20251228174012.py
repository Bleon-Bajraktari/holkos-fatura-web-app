...
class InvoiceListView(ctk.CTkFrame):
    ...
    def load_invoices(self):
        """Ngarkon dhe shfaq faturat në një listë të vetme (pa grupim)"""
        # Pastrimi
        ...
        # Shfaqja në një listë të vetme (one-column list layout)
        for inv in invoices:
            self.add_invoice_row(self.scroll_container, inv)
    ...
    def add_invoice_row(self, parent, inv):
        ...
        # Grid weights (Sinkronizuar me Header)
        row_frame.grid_columnconfigure(0, weight=10) # Numri
        row_frame.grid_columnconfigure(1, weight=10) # Data
        row_frame.grid_columnconfigure(2, weight=25) # Klienti
        row_frame.grid_columnconfigure(3, weight=12) # Pa TVSH
        row_frame.grid_columnconfigure(4, weight=10) # TVSH
        row_frame.grid_columnconfigure(5, weight=13) # Totali
        row_frame.grid_columnconfigure(6, weight=20) # Veprimet
        ...
        # 4. Pa TVSH
        subtotal_val = f"{float(inv.get('subtotal', 0)):,.2f} €".replace(",", " ")
        ctk.CTkLabel(row_frame, text=subtotal_val, font=ctk.CTkFont(family="Inter", size=13), text_color=self.text_dim).grid(row=0, column=3, sticky="w", padx=10)
        
        # 5. TVSH
        vat_val = f"{float(inv.get('vat_amount', 0)):,.2f} €".replace(",", " ")
        ctk.CTkLabel(row_frame, text=vat_val, font=ctk.CTkFont(family="Inter", size=13), text_color=self.text_dim).grid(row=0, column=4, sticky="w", padx=10)
        
        # 6. Totali
        total_val = f"{float(inv.get('total', 0)):,.2f} €".replace(",", " ")
        ctk.CTkLabel(row_frame, text=total_val, font=ctk.CTkFont(family="Inter", size=13, weight="bold"), text_color=self.text_main).grid(row=0, column=5, sticky="w", padx=10)
        
        # 7. Veprimet
        actions = ctk.CTkFrame(row_frame, fg_color="transparent")
        actions.grid(row=0, column=6, sticky="e", padx=15)
    ...
    def handle_pdf_action(self, inv_id, path):
        ...
        if path and os.path.exists(path):
            try:
                ...
                # This error handling seems misplaced for os.startfile,
                # but following the instruction to insert it here.
                if "Duplicate entry" in str(e):
                    # 'invoice' object is not available here, assuming it's a placeholder
                    # and the intent was to show a generic error or that this code
                    # was meant for a different context (e.g., invoice saving).
                    # For os.startfile, a "Duplicate entry" error is highly unlikely.
                    messagebox.showerror("Gabim", f"Numri i faturës ekziston tashmë! Ju lutem përdorni një numër tjetër.")
                else:
