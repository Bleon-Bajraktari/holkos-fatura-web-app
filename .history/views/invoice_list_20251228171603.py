...
class InvoiceListView(ctk.CTkFrame):
    ...
    def add_invoice_item(self, parent, invoice):
        ...
        cols = [
            (invoice['invoice_number'], 0.12),
            (invoice['date'].strftime("%d.%m.%Y") if invoice['date'] else "", 0.12),
            (invoice.get('client_name', ''), 0.30),
            (f"{float(invoice.get('total', 0)):,.2f} €".replace(",", " "), 0.12),
        ...
        for text, weight in cols:
            ...
            # Special formatting for Status and Total
            if weight == 0.10: # Status
            ...
            elif "€" in str(text): # Total
                 lbl = ctk.CTkLabel(item_frame, text=text, font=ctk.CTkFont(size=12, weight="bold"))
            else:
                lbl = ctk.CTkLabel(item_frame, text=text, font=ctk.CTkFont(size=12))
            
            # Use specific relx positions to match header
            lbl.place(relx=current_relx, rely=0.5, anchor="w")
            current_relx += weight
        
        # Veprimet
        actions = ctk.CTkFrame(item_frame, fg_color="transparent")
        actions.place(relx=0.78, rely=0.5, anchor="w") # Start actions a bit earlier
