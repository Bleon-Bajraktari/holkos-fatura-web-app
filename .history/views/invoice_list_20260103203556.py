...
class InvoiceListView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.current_invoices = [] # Ruaj faturat aktuale për eksport
    ...
    def create_widgets(self):
        ...
        self.btn_refresh = ctk.CTkButton(
            ...
            fg_color=self.bg_secondary, text_color=self.text_main, hover_color="#DEE2E6",
        ...
        self.btn_export = ctk.CTkButton(
            self.top_section, text="📥 Eksporto CSV", width=120, height=32,
            fg_color="#2ECC71", hover_color="#27AE60", text_color="white",
            font=ctk.CTkFont(size=12, weight="bold"), command=self.export_to_csv, cursor="hand2"
        )
    ...
    def load_invoices(self):
        ...
        self.current_invoices = invoices if invoices else []
    ...
    def add_invoice_row(self, parent, inv):
        ...
        # STATUSI dhe VEPRIMET
        actions = ctk.CTkFrame(row_frame, fg_color="transparent")
        actions.grid(row=0, column=5, sticky="e", padx=10)
        
        # Status Icon (Dot)
        status = inv.get('status', 'draft')
        status_color = "#2ECC71" if status == 'paid' else "#F39C12" # Green if paid, Orange if sent
        status_tooltip = "E Paguar" if status == 'paid' else "E Papaguar"
        
        status_btn = ctk.CTkButton(actions, text="✔" if status == 'paid' else "Paguaj", width=55 if status != 'paid' else 30, height=24, 
                                 fg_color=status_color, font=ctk.CTkFont(size=10, weight="bold"),
                                 state="disabled" if status == 'paid' else "normal",
                                 command=lambda i=inv['id']: self.mark_as_paid(i))
    ...
    def mark_as_paid(self, inv_id):
        """Shënon faturën si të paguar"""
        if messagebox.askyesno("Konfirmim", "A dëshironi ta shënoni këtë faturë si të PAGUAR?"):
            query = "UPDATE invoices SET status = 'paid' WHERE id = %s"
            if self.db.execute_query(query, (inv_id,)):
                self.load_invoices()
                messagebox.showinfo("Sukses", "Statusi u përditësua!")

    def export_to_csv(self):
        """Eksporton listën aktuale në CSV"""
        if not self.current_invoices:
            messagebox.showwarning("Kujdes", "Nuk ka fatura për të eksportuar!")
            return
            
        filename = f"Faturat_Export_{date.today().strftime('%Y-%m-%d')}.csv"
        ...
        if filepath:
            try:
                with open(filepath, 'w', newline='', encoding='utf-8') as f:
                    writer = csv.writer(f)
                    # Header
                    writer.writerow(["Nr Faturës", "Data", "Klienti", "Total Pa TVSH", "TVSH %", "Vlera TVSH", "TOTAL", "Statusi"])
                    ...
                    for inv in self.current_invoices:
                        writer.writerow([
                            ...
                            f"{float(inv['vat_percentage']):.2f}",
                            f"{float(inv['vat_amount']):.2f}",
                            f"{float(inv['total']):.2f}",
                            inv['status']
                        ])
                messagebox.showinfo("Sukses", f"Eksporti u krye me sukses në:\n{filepath}")
            except Exception as e:
                messagebox.showerror("Gabim", f"Eksporti dështoi: {str(e)}")
