        ...
        if invoice.save():
            ...
            try:
                ...
                if action == 'pdf':
                    ...
                    if not comp.email:
                        messagebox.showwarning("Kujdes", "Fatura u ruajt dhe u hap PDF, por kompania nuk ka email të konfiguruar!")
                    else:
                        success, msg = EmailService.send_invoice(invoice, comp.email, output_path, is_to_company=True)
                        if success: messagebox.showinfo("Sukses", f"Fatura u ruajt, u hap PDF dhe u dërgua te {comp.email}!")
