...
class InvoiceListView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Merr vitet e disponueshme nga DB (Bonus: do bëjmë një query të shpejtë ose hardcode për tani + dinamike)
        available_years = [str(y) for y in range(date.today().year, 2023, -1)] 
        if self.current_year not in available_years: available_years.insert(0, self.current_year)
