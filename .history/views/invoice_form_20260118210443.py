...
class InvoiceFormView(ctk.CTkFrame):
    def __init__(self, parent, invoice_id=None, is_clone=False):
        ...
        if invoice_id:
            ...
            if self.is_clone:
                # Nëse është klon, pastro ID-në që të ruhet si e re
    ...
    def load_invoice_data(self):
        ...
        if self.is_clone:
            # Klonim: Data sot, Numër i ri, Afat bosh
    ...
    def get_invoice_data(self):
        ...
        if not client_text:
             messagebox.showerror("Gabim", "Ju lutem zgjidhni një klient!"); return None
        
        # Nxirr emrin dhe numrin unik nga formati "Emri (Numri)"
        client_id = None
        clients = Client.get_all(self.db)
        
        if " (" in client_text:
            name_part = client_text.split(" (")[0].strip()
            unique_part = client_text.split(" (")[1].replace(")", "").strip()
            # Kërko për përputhje të plotë (Emër + Numër Unik)
            client_id = next((c['id'] for c in clients if c['name'].strip() == name_part and str(c['unique_number']).strip() == unique_part), None)
            
        # Nëse nuk u gjet me formatin e mësipërm, provo vetëm me emër
        if not client_id:
            name_to_search = client_text.split(" (")[0].strip()
            client_id = next((c['id'] for c in clients if c['name'].strip() == name_to_search), None)

        if not client_id:
            print(f"DEBUG: Klienti '{client_text}' nuk u gjet në listën me {len(clients)} klientë.")
            self.client_entry.configure(border_color="#E74C3C")
            messagebox.showerror("Gabim", f"Klienti '{client_text}' nuk u gjet në databazë!")
            return None
