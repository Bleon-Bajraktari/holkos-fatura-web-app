        ...
        if invoice.save():
            ...
            try:
                ...
                if action == 'pdf':
                    ...
                    if not comp.email:
                        messagebox.showwarning("Kujdes", "Fatura u ruajt, por kompania nuk ka email të konfiguruar te Cilësimet!")
                    else:
                        success, msg = EmailService.send_invoice(invoice, comp.email, output_path, is_to_company=True)
                        if success: messagebox.showinfo("Sukses", f"Fatura u ruajt dhe u dërgua te {comp.email}!")
