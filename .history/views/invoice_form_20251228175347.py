...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def show_suggestions(self, matches):
        """Shfaq listën e sugjerimeve me dizajni premium dhe pozicionim fiks"""
        # Llogarit lartësinë dinamike
        row_height = 42
        max_h = 220
        needed_h = (len(matches) * row_height) + 10
        final_h = min(needed_h, max_h)

        if not self.suggestions_frame:
            self.suggestions_frame = ctk.CTkScrollableFrame(
                self, 
                width=self.client_entry.winfo_width(), 
                height=final_h, 
                fg_color="#FFFFFF", 
                border_width=1, 
                border_color="#D1D1D1",
            ...
            self.suggestions_frame.configure(height=final_h)
    ...
    def _position_suggestions(self):
        """Pozicionon saktë frame-in e sugjerimeve relative ndaj dritares kryesore"""
        if not self.suggestions_frame: return
        
        # Sigurohu që gjerësia përputhet fiks me entry
        entry_w = self.client_entry.winfo_width()
        if entry_w > 1: # Shmangi rastet kur widget nuk është vizatuar ende
            self.suggestions_frame.configure(width=entry_w - 20)
        
        # Llogarit pozicionin absolut relative ndaj 'self'
        try:
            # Pozicioni i Entry në ekran
            entry_rx = self.client_entry.winfo_rootx()
            entry_ry = self.client_entry.winfo_rooty()
            
            # Pozicioni i Form-it në ekran
            self_rx = self.winfo_rootx()
            self_ry = self.winfo_rooty()
            
            # Diferenca na jep pozicionin relativ për .place()
            x = entry_rx - self_rx
            y = (entry_ry - self_ry) + self.client_entry.winfo_height() + 2
            
            self.suggestions_frame.place(x=x, y=y)
            self.suggestions_frame.lift()
        except:
            # Në rast se winfo nuk është gati
            pass
