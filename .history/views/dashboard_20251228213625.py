"""
Dashboard view - v1.1.0 (Zoom Out)
"""
...
class DashboardView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        """Krijon ndërfaqen premium të dashboard-it - Zoom Out"""
        # 1. Top Header Section (Reduktuar)
        header_container = ctk.CTkFrame(self, fg_color="transparent")
        header_container.pack(fill="x", padx=30, pady=(25, 15))
        
        # Title only (Size 22 në vend të 28)
        ctk.CTkLabel(
            header_container, 
        ...
        # 2. Statistics Cards Grid (Më kompakte)
        ...
        # 3. Recent Activity Section (Reduktuar)
        main_content = ctk.CTkFrame(self, fg_color="transparent")
        main_content.pack(fill="both", expand=True, padx=30, pady=(15, 25))
        ...
        # Scrollable rows
        self.rows_container = ctk.CTkScrollableFrame(table_main, fg_color="transparent", height=200)
        self.rows_container.pack(fill="both", expand=True)

    def create_stat_card(self, index, label_text, key, icon, color):
        """Krijon një kartelë statistikore - Zoom Out"""
        card = ctk.CTkFrame(self.stats_container, fg_color="#FFFFFF", height=100, corner_radius=12, border_width=1, border_color=self.border_color)
        ...
        icon_box = ctk.CTkFrame(card, fg_color=bg_color, width=38, height=38, corner_radius=8)
    ...
    def render_recent_rows(self, invoices):
        ...
        for i, inv in enumerate(invoices):
            ...
            ctk.CTkLabel(row, text=client, font=ctk.CTkFont(family="Inter", size=11), text_color=self.text_main).grid(row=0, column=2, padx=12, sticky="w")
