"""
Formulari i Faturës - v1.1.0 (Zoom Out)
"""
import os
...
class InvoiceFormView(ctk.CTkFrame):
    """Formulari për krijimin dhe redaktimin e faturave - Zoom Out"""
    ...
    def create_widgets(self):
        ...
        # 2. Informacione Bazë (Compact Card)
        ...
        self.item_col_weights = [4, 1.5, 1.5, 2, 0.5] 
        ...
        h_text = ["PËRSHKRIMI", "SASIA", "ÇMIMI", "NËNTOTALI", ""]
        ...
        self.btn_save_only = ctk.CTkButton(
            summary_outer, text="Ruaj", command=lambda: self.process_save(show_pdf=False),
            fg_color="#34495E", hover_color="#2C3E50", text_color="#FFFFFF",
            font=ctk.CTkFont(family="Inter", size=13, weight="bold"), width=120, height=38, corner_radius=8, cursor="hand2"
        )
        self.btn_save_only.pack(side="left", padx=(0, 10), pady=(60, 0))

        self.btn_save_pdf = ctk.CTkButton(
            summary_outer, text="Ruaj & PDF", command=lambda: self.process_save(show_pdf=True),
            fg_color=self.primary_color, hover_color="#E65C00",
            font=ctk.CTkFont(family="Inter", size=13, weight="bold"), width=150, height=38, corner_radius=8, cursor="hand2"
        )
        self.btn_save_pdf.pack(side="left", pady=(60, 0))
    ...
    def _add_form_field(self, parent, label, row, col, is_search=False):
        ...
        if is_search:
            ...
            btn_new = ctk.CTkButton(parent, text="Klient i ri", command=self.open_new_client, width=80, height=32, fg_color="transparent", border_width=1, text_color=self.primary_color, border_color=self.primary_color, hover_color="#FFF4ED", font=ctk.CTkFont(size=11, weight="bold"), cursor="hand2")
            btn_new.grid(row=row, column=2, padx=10, sticky="s", pady=(0, 1))

    def _add_row_field(self, parent, label, attr_name, col, default=""):
        container = ctk.CTkFrame(parent, fg_color="transparent")
        container.grid(row=row, column=col, padx=8, sticky="ew") # Wait, 'row' is not defined here in original? Corrected to 0.
    ...
    def add_item_row(self):
        ...
        subtotal_label.grid(row=0, column=3, padx=12, sticky="w")
        btn_del = ctk.CTkButton(row_frame, text="✕", width=28, height=28, fg_color="#FFF0F0", text_color="#E74C3C", hover_color="#FFE5E5", corner_radius=6, command=lambda r=row_id: self.remove_item_row(r), cursor="hand2")
    ...
    def process_save(self, show_pdf=False):
        ...
        target_btn = self.btn_save_pdf if show_pdf else self.btn_save_only; original_text = target_btn.cget("text")
        ...
        if invoice.save():
            ...
            try:
                ...
                if show_pdf: os.startfile(output_path); messagebox.showinfo("Sukses", "U ruajt!")
                else: messagebox.showinfo("Sukses", "U ruajt!")
            except Exception as e: messagebox.showerror("Gabim", f"Gabim PDF: {str(e)}")
            finally: target_btn.configure(text=original_text, state="normal")
        else: target_btn.configure(text=original_text, state="normal"); messagebox.showerror("Gabim", "Gabim ruajtje!")
