"""
Formulari i Faturës - v1.1.1 (Fix & Zoom Out)
"""
import os
...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.btn_save_only = ctk.CTkButton(summary_outer, text="Ruaj", command=lambda: self.process_save(show_pdf=False), fg_color="#34495E", hover_color="#2C3E50", text_color="#FFFFFF", font=ctk.CTkFont(family="Inter", size=13, weight="bold"), width=120, height=38, corner_radius=8, cursor="hand2")
        self.btn_save_only.pack(side="left", padx=(0, 10), pady=(60, 0))

        self.btn_save_pdf = ctk.CTkButton(summary_outer, text="Ruaj & PDF", command=lambda: self.process_save(show_pdf=True), fg_color=self.primary_color, hover_color="#E65C00", font=ctk.CTkFont(family="Inter", size=13, weight="bold"), width=150, height=38, corner_radius=8, cursor="hand2")
    ...
    def add_item_row(self):
        ...
        ctk.CTkButton(row_frame, text="✕", width=28, height=28, fg_color="#FFF0F0", text_color="#E74C3C", hover_color="#FFE5E5", corner_radius=6, command=lambda r=row_id: self.remove_item_row(r), cursor="hand2").grid(row=0, column=4, padx=5)
