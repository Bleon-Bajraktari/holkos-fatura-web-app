...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        """Krijon dritaren e faturimit me dizajnin ultra-premium"""
        # 1. Header (Titulli)
        ...
        h_text = ["PËRSHKRIMI", "SASIA (M²)", "ÇMIMI", "NËNTOTALI", ""]
        ...
        self.btn_save_only.pack(side="left", padx=(0, 15), pady=(100, 0))
        ...
        self.btn_save_pdf.pack(side="left", pady=(100, 0))
    ...
    def _add_form_field(self, parent, label, row, col, is_search=False):
        ...
        if is_search:
            ...
            # Container i Sugjerimeve i integruar brenda rreshtit
    ...
    def show_suggestions(self, matches):
        """Shfaq listën e sugjerimeve me alinjim të përsosur"""
        # Pastrimi
        for widget in self.suggestions_container.winfo_children():
            widget.destroy()

        # Shfaq containerin
        self.suggestions_container.pack(fill="x", pady=(5, 0))
        
        # Area e sugjerimeve me stil modern
        results_frame = ctk.CTkScrollableFrame(
            self.suggestions_container, 
            height=min(len(matches) * 45, 200), 
            fg_color="#FFFFFF", 
            border_width=1, 
            border_color=self.border_color,
            corner_radius=10
        ...
        for name in matches:
            ...
            item_frame = ctk.CTkFrame(results_frame, fg_color="transparent", height=42, cursor="hand2")
    ...
    def hide_suggestions(self):
        ...
        if hasattr(self, 'suggestions_container'):
            self.suggestions_container.pack_forget()
    ...
    def open_new_client(self):
        """Hap dritaren për klient të ri mbi dritaren aktuale"""
        from views.client_manager import ClientManagerView
        
        # Krijo dritaren
        client_window = ctk.CTkToplevel(self)
        client_window.title("Shto Klient të Ri")
        client_window.geometry("900x700")
        
        # Beje dritaren të qëndrojë sipër - MODAL
        client_window.transient(self.winfo_toplevel())
        client_window.grab_set()
        client_window.focus_set()
        
        # Centering (Opcionale por i rekomanduar)
        ...
        # Përditëso listën e klientëve pas mbylljes
        client_window.protocol("WM_DELETE_WINDOW", lambda: (
            client_window.grab_release(),
    ...
    def get_invoice_data(self):
        ...
        if not client_id:
            self.client_entry.configure(border_color="#E74C3C") # Red border feedback
            messagebox.showerror("Gabim", "Ju lutem zgjidhni një klient nga lista!")
            return None
        
        self.client_entry.configure(border_color=self.border_color) # Reset border
    ...
    def process_save(self, show_pdf=False):
        ...
        original_text = self.btn_save_pdf.cget("text") if show_pdf else self.btn_save_only.cget("text")
        target_btn = self.btn_save_pdf if show_pdf else self.btn_save_only
        
        target_btn.configure(text="Duke u ruajtur...", state="disabled")
