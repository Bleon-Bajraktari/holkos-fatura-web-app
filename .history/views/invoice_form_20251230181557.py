        ...
        if invoice.save():
            ...
            try:
                ...
                if action == 'pdf':
                    ...
                    # Merr email-in e klientit
                    client = Client.get_by_id(invoice.client_id, self.db)
                    if not client.email:
                        messagebox.showwarning("Kujdes", "Fatura u ruajt, por klienti nuk ka email!")
                    else:
                        success, msg = EmailService.send_invoice(invoice, client.email, output_path)
                        if success: messagebox.showinfo("Sukses", f"Fatura u ruajt dhe u dërgua me email te {client.email}!")
