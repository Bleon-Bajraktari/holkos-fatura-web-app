...
class OfferFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(header_frame, text="Struktura e Ofertës (Blloqe)", font=ctk.CTkFont(size=11, weight="bold"), text_color=self.text_secondary).pack(side="left", padx=10)
    ...
    def add_aligned_row(self, initial_data=None):
        ...
        row_frame.columnconfigure(1, weight=1) # Calculations/Modules
        ...
        right_side_container = ctk.CTkFrame(row_frame, fg_color="transparent")
        right_side_container.grid(row=0, column=1, sticky="ne", padx=10, pady=10)
        
        modules_header = ctk.CTkFrame(right_side_container, fg_color="transparent")
        modules_header.pack(fill="x", pady=(0, 5))
        
        ctk.CTkLabel(modules_header, text="Modulet:", font=ctk.CTkFont(size=10, weight="bold"), text_color=self.text_secondary).pack(side="left")
        
        # Add Module Button
        add_mod_btn = ctk.CTkButton(modules_header, text="+", width=20, height=20, font=ctk.CTkFont(size=12, weight="bold"), fg_color=self.primary_color, hover_color="#E65C00", command=lambda: self.add_module_to_row(row_id))
        add_mod_btn.pack(side="right")
        
        modules_list_frame = ctk.CTkFrame(right_side_container, fg_color="transparent")
        modules_list_frame.pack(fill="x")
        
        # Totali i rreshtit (Optional, hidden by default unless there are price/qty modules)
        subtotal_label = ctk.CTkLabel(right_side_container, text="0.00 €", font=ctk.CTkFont(size=11, weight="bold"), text_color=self.primary_color)
        subtotal_label.pack(side="bottom", anchor="e", pady=(5, 0))
        
        # Delete Row Button
        del_row_btn = ctk.CTkButton(row_frame, text="✕", width=25, height=25, fg_color="transparent", text_color="#E74C3C", hover_color="#FFF0F0", command=lambda r=row_id: self.remove_item_row(r))
        del_row_btn.grid(row=0, column=2, sticky="ne", padx=5, pady=5)
        ...
        # Add default modules if no initial data
        if not initial_data:
            self.add_module_to_row(row_id, "Sasia", "")
            self.add_module_to_row(row_id, "Çmimi", "€")
        else:
            # Loading will happen later via separate call or we can do it here
            pass
    ...
    def add_module_to_row(self, row_id, label_text="", unit_text=""):
        ...
        mod_id = len(item['modules']) + 1
        mod_frame = ctk.CTkFrame(item['modules_container'], fg_color="transparent")
        mod_frame.pack(fill="x", pady=2)
        
        # Entry for Value
        val_entry = ctk.CTkEntry(mod_frame, width=60, height=26, font=ctk.CTkFont(size=11), placeholder_text="Vlera")
        val_entry.pack(side="left", padx=2)
        
        # Entry for Unit/Label
        unit_entry = ctk.CTkEntry(mod_frame, width=40, height=26, font=ctk.CTkFont(size=10), placeholder_text="Njësia", fg_color="#F8F9FA")
        if unit_text: unit_entry.insert(0, unit_text)
        elif label_text: unit_entry.insert(0, label_text)
        unit_entry.pack(side="left", padx=2)
        
        # Delete Module Button
        ctk.CTkButton(mod_frame, text="-", width=18, height=18, fg_color="#FBE9E7", text_color="#D84315", hover_color="#FFCCBC", corner_radius=4, command=lambda: self.remove_module_from_row(row_id, mod_id)).pack(side="left", padx=5)
        ...
        # Binding for subtotal update
        val_entry.bind("<KeyRelease>", lambda e: self.update_row_subtotal(row_id))
        unit_entry.bind("<KeyRelease>", lambda e: self.update_row_subtotal(row_id))
    ...
    def update_row_subtotal(self, row_id):
        ...
        if len(vals) >= 2:
            ...
            item['subtotal_label'].pack(side="bottom", anchor="e", pady=(5, 0))
        else:
            # If only one or none, maybe it's just a total
            if len(vals) == 1:
                item['subtotal_label'].configure(text=f"{vals[0]:,.2f} €".replace(",", " "))
                item['subtotal_label'].pack(side="bottom", anchor="e", pady=(5, 0))
            else:
                # Hide it if no numbers
                item['subtotal_label'].configure(text="0.00 €")
                # item['subtotal_label'].pack_forget()
    ...
    def load_offer_data(self):
        ...
        for item_data in self.offer.items:
            self.add_aligned_row(initial_data=item_data)
            last_item = self.items[-1]
            last_item['desc_text'].insert("0.0", item_data['description'])
            
            # Clear default modules added by add_aligned_row
            for mod in list(last_item['modules']):
                mod['container'].destroy()
            last_item['modules'] = []
            ...
            if custom_attr and isinstance(custom_attr, list):
                ...
                # Fallback to standard Qty/Price if no custom_attributes
    ...
    def get_offer_data(self):
        ...
        for item_entry in self.items:
            ...
            for mod in item_entry['modules']:
                val = mod['val_entry'].get()
                unit = mod['unit_entry'].get()
                if val:
            ...
            # For backward compatibility and summary, we still store a single Qty/Price if possible
            # Or just store the subtotal and description
            qty = Decimal("1.00")
            price = Decimal("0.00")
            
            if len(row_vals) >= 2:
                # Multiply all numeric modules for subtotal
                sub = 1.0
            ...
            items.append({
                'description': desc,
                'unit': "", # Will use custom_attributes instead
                'quantity': qty,
                'unit_price': price,
        ...
        for it in items: 
            offer.add_item(it['description'], it['unit'], it['quantity'], it['unit_price'], it['custom_attributes'])
