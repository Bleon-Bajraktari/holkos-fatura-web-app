...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def refresh_clients(self):
        ...
        self.all_client_names = self.get_client_names()
        self.client_combo.configure(values=self.all_client_names)
    
    def on_client_search(self, event):
        """Filtra klientët sipas kërkimit live"""
        search_term = self.client_var.get().lower()
        
        if not search_term:
            filtered = self.all_client_names
        else:
            # Filtro klientët që fillojnë me këtë germë ose përmbajnë tekstin
            filtered = [name for name in self.all_client_names if search_term in name.lower()]
            
        self.client_combo.configure(values=filtered if filtered else ["Asnjë rezultat"])
        
        # Opcionale: Hape dropdown nëse nuk është hapur (Trick në CTK)
        # self.client_combo._canvas.focus_set() # Nuk funksionon gjithmonë mirë
