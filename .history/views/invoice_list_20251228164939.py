...
class InvoiceListView(ctk.CTkFrame):
    ...
    def add_invoice_item(self, parent, invoice):
        ...
        # Butonat e veprimeve me ikona pseudo (tekst)
        btn_view = ctk.CTkButton(actions, text="👁", width=30, height=30, fg_color="transparent", text_color=self.primary_color, hover_color=self.bg_secondary, command=lambda: self.view_invoice(inv_id))
        btn_view.pack(side="left", padx=2)
        
        btn_edit = ctk.CTkButton(actions, text="✎", width=30, height=30, fg_color="transparent", text_color="orange", hover_color=self.bg_secondary, command=lambda: self.edit_invoice(inv_id))
        btn_edit.pack(side="left", padx=2)
        
        btn_pdf = ctk.CTkButton(actions, text="PDF", width=40, height=30, fg_color=self.primary_color, hover_color=self.hover_color, font=ctk.CTkFont(size=10, weight="bold"), command=lambda: self.generate_pdf(inv_id))
        btn_pdf.pack(side="left", padx=2)
        
        btn_delete = ctk.CTkButton(actions, text="×", width=30, height=30, fg_color="transparent", text_color="red", hover_color=self.bg_secondary, font=ctk.CTkFont(size=16, weight="bold"), command=lambda: self.delete_invoice(inv_id))
    ...
    def view_invoice(self, invoice_id):
        ...
        if invoice:
            ...
            view_window.after(100, lambda: view_window.focus()) # Fix focus
    ...
    def generate_pdf(self, invoice_id):
        ...
        try:
            ...
            messagebox.showinfo("Sukses", f"PDF u gjenerua me sukses!\nZgjedhni skedarin: {output_path}")
            
            # Provo hapjen e PDF automatikisht
            import os
