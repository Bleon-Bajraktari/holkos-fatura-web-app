...
class ClientManagerView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Table Header
        header_table = ctk.CTkFrame(table_container, fg_color="#FFFFFF", height=45)
        header_table.pack(fill="x", pady=(0, 1))
        
        header_columns = [("EMRI", 0.25), ("ADRESA", 0.20), ("NUMRI UNIK", 0.15), ("TELEFONI", 0.15), ("EMAIL", 0.15), ("VEPRIMET", 0.10)]
        for text, weight in header_columns:
            lbl = ctk.CTkLabel(
                header_table, 
                text=text, 
            ...
            # Përdorim grid për kontroll fiks ose x-relative
            # Do të përdorim sistemin e thjeshtë me grid weight
        
        # Përdorim grid në header për rreshtim
        header_table.grid_columnconfigure(0, weight=3) # Emri
        header_table.grid_columnconfigure(1, weight=2) # Adresa
        header_table.grid_columnconfigure(2, weight=2) # Numri
        header_table.grid_columnconfigure(3, weight=2) # Telefoni
        header_table.grid_columnconfigure(4, weight=2) # Email
        header_table.grid_columnconfigure(5, weight=1) # Actions
        
        cols = ["EMRI", "ADRESA", "NUMRI UNIK", "TELEFONI", "EMAIL", ""]
        for i, col in enumerate(cols):
            ctk.CTkLabel(header_table, text=col, font=ctk.CTkFont(size=11, weight="bold"), text_color="#666666").grid(row=0, column=i, padx=20, sticky="w")
    ...
    def add_client_row(self, client):
        """Shton një rresht premium në listë"""
        row = ctk.CTkFrame(self.scroll_container, fg_color="#FFFFFF", height=60, corner_radius=8)
        row.pack(fill="x", pady=2)
        row.pack_propagate(False)
        
        row.grid_columnconfigure(0, weight=3) # Emri
        row.grid_columnconfigure(1, weight=2) # Adresa
        row.grid_columnconfigure(2, weight=2) # Numri
        row.grid_columnconfigure(3, weight=2) # Telefoni
        row.grid_columnconfigure(4, weight=2) # Email
        row.grid_columnconfigure(5, weight=1) # Actions
        ...
        # Të dhënat
        ctk.CTkLabel(row, text=client['name'], font=ctk.CTkFont(family="Inter", weight="bold"), text_color=self.text_main).grid(row=0, column=0, padx=20, sticky="w")
        ctk.CTkLabel(row, text=client.get('address', '-'), font=ctk.CTkFont(family="Inter"), text_color=self.text_secondary).grid(row=0, column=1, padx=20, sticky="w")
        ctk.CTkLabel(row, text=client.get('unique_number', '-'), font=ctk.CTkFont(family="Inter"), text_color=self.text_secondary).grid(row=0, column=2, padx=20, sticky="w")
        ctk.CTkLabel(row, text=client.get('phone', '-'), font=ctk.CTkFont(family="Inter"), text_color=self.text_secondary).grid(row=0, column=3, padx=20, sticky="w")
        ctk.CTkLabel(row, text=client.get('email', '-'), font=ctk.CTkFont(family="Inter"), text_color=self.text_secondary).grid(row=0, column=4, padx=20, sticky="w")
        
        # Veprimet
        actions = ctk.CTkFrame(row, fg_color="transparent")
        actions.grid(row=0, column=5, padx=10, sticky="e")
        
        # Edit btn
        btn_edit = ctk.CTkButton(
            actions, text="✎", width=35, height=35, fg_color="#F1F1F1", text_color=self.text_main,
            hover_color="#E5E5E5", command=lambda c_id=client['id']: self.edit_client(c_id)
        )
        btn_edit.pack(side="left", padx=2)
        
        # Delete btn
        btn_del = ctk.CTkButton(
            actions, text="🗑", width=35, height=35, fg_color="#FFF0F0", text_color="#E74C3C",
            hover_color="#FFE5E5", command=lambda c_id=client['id']: self.delete_client(c_id)
        )
