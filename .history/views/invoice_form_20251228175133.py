...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def show_suggestions(self, matches):
        """Shfaq listën e sugjerimeve si overlay"""
        if not self.suggestions_frame:
            # Krijojmë frame-in e sugjerimeve nëse nuk ekziston
        ...
        # Pastro sugjerimet e vjetra
        for widget in self.suggestions_frame.winfo_children():
            widget.destroy()
            
        # Shto sugjerimet e reja
        for name in matches:
            btn = ctk.CTkButton(
                ...
                hover_color="#F0F0F0",
                height=30,
                command=lambda n=name: self.select_client(n)
            )
            btn.pack(fill="x", padx=2, pady=1)
            
        # Poziciono frame-in direkt poshtë entry-t
        # Root koordinatat e entry-t
        x = self.client_entry.winfo_x()
        y = self.client_entry.winfo_y() + self.client_entry.winfo_height() + 10
