...
class InvoiceListView(ctk.CTkFrame):
    ...
    def add_invoice_row(self, parent, inv):
        ...
        status_color = "#2ECC71" if is_paid else "#E74C3C" # Green / Red
        ...
        status_container.grid(row=0, column=3, sticky="w", padx=5)
        ...
        # Butoni Toggle Status (✓)
        toggle_icon = "↩" if is_paid else "✓"
        toggle_color = "#95A5A6" if is_paid else "#27AE60"
        toggle_hover = "#7F8C8D" if is_paid else "#2ECC71"
        tooltip = "Shëno e Papaguar" if is_paid else "Shëno e Paguar"
