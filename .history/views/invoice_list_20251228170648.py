...
class InvoiceListView(ctk.CTkFrame):
    ...
    def add_invoice_item(self, parent, invoice):
        ...
        # Butonat e veprimeve
        # View
        btn_view = ctk.CTkButton(actions, text="👁", width=30, height=30, fg_color="transparent", text_color="gray20", hover_color="gray90", font=ctk.CTkFont(size=14), command=lambda: self.view_invoice(inv_id))
        btn_view.pack(side="left", padx=2)
        
        # Edit
        btn_edit = ctk.CTkButton(actions, text="✎", width=30, height=30, fg_color="transparent", text_color="blue", hover_color="lightblue", font=ctk.CTkFont(size=14), command=lambda: self.edit_invoice(inv_id))
        ...
        btn_pdf_color = "#4CAF50" if pdf_exists else self.primary_color # Green if exists
        ...
        # Delete
        btn_del = ctk.CTkButton(actions, text="×", width=30, height=30, fg_color="transparent", text_color="red", hover_color="#FFCDD2", font=ctk.CTkFont(size=16, weight="bold"), command=lambda: self.delete_invoice(inv_id))
