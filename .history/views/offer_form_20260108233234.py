...
class OfferFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.main_content = ctk.CTkFrame(self.scrollable_frame, fg_color="transparent")
        self.main_content.pack(fill="both", expand=True, padx=20, pady=10)
        self.main_content.columnconfigure(0, weight=3) # Left: Description (wider)
        self.main_content.columnconfigure(1, weight=1) # Right: Items (narrower)
        
        # LEFT SIDE: Description
        self.left_panel = ctk.CTkFrame(self.main_content, fg_color=self.bg_card, corner_radius=12, border_width=1, border_color=self.border_color)
        self.left_panel.grid(row=0, column=0, sticky="nsew", padx=(0, 10))
        
        ctk.CTkLabel(self.left_panel, text="Përshkrimi i Ofertës / Kushtet:", font=ctk.CTkFont(family="Inter", size=11, weight="bold"), text_color=self.text_main).pack(anchor="w", padx=15, pady=(15, 5))
        
        self.description_text = ctk.CTkTextbox(self.left_panel, font=ctk.CTkFont(size=12), fg_color="#FBFBFB", border_color=self.border_color, border_width=1, corner_radius=8)
        self.description_text.pack(fill="both", expand=True, padx=15, pady=(0, 15))
        
        # RIGHT SIDE: Items Modules
        self.right_panel = ctk.CTkFrame(self.main_content, fg_color=self.bg_card, corner_radius=12, border_width=1, border_color=self.border_color)
        ...
        ctk.CTkLabel(right_header, text="Kalkulimet (Opsionale):", font=ctk.CTkFont(family="Inter", size=11, weight="bold"), text_color=self.text_main).pack(side="left")
        ctk.CTkButton(right_header, text="+ Shto", command=self.add_module_item, width=60, height=24, font=ctk.CTkFont(size=11, weight="bold")).pack(side="right")
        
        self.modules_container = ctk.CTkScrollableFrame(self.right_panel, fg_color="transparent")
        self.modules_container.pack(fill="both", expand=True, padx=5, pady=5)
        
        # Footer in Right Panel for Total
        self.right_footer = ctk.CTkFrame(self.right_panel, fg_color="#F8F9FA", height=40, corner_radius=8)
        self.right_footer.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(self.right_footer, text="TOTALI:", font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=10)
        self.total_label = ctk.CTkLabel(self.right_footer, text="0,00 €", font=ctk.CTkFont(size=13, weight="bold"), text_color=self.primary_color)
        self.total_label.pack(side="right", padx=10)

        # Action Buttons (Floating at bottom right of main scrollable, or keep outside)
    ...
    def add_module_item(self):
        row_id = len(self.items) + 1
        card = ctk.CTkFrame(self.modules_container, fg_color="#FFFFFF", border_width=1, border_color="#E0E0E0", corner_radius=6)
        card.pack(fill="x", pady=3)
        
        # Line 1: Label/Description
        # ctk.CTkLabel(card, text="Përshkrimi:", font=ctk.CTkFont(size=10)).pack(anchor="w", padx=5, pady=(5,0))
        desc_entry = ctk.CTkEntry(card, height=28, placeholder_text="P.sh. Dritare...", font=ctk.CTkFont(size=11), border_width=0, fg_color="transparent")
        desc_entry.pack(fill="x", padx=5, pady=(2, 0))
        
        # Line 2: Inputs (Qty, Unit, Price, Total)
        vals_frame = ctk.CTkFrame(card, fg_color="transparent")
        vals_frame.pack(fill="x", padx=5, pady=5)
        
        # Unit
        unit_entry = ctk.CTkEntry(vals_frame, width=50, height=26, font=ctk.CTkFont(size=11), placeholder_text="m2")
        unit_entry.pack(side="left", padx=2)
        unit_entry.insert(0, "m2")
        
        # Qty
        qty_entry = ctk.CTkEntry(vals_frame, width=50, height=26, font=ctk.CTkFont(size=11), placeholder_text="Sasia")
        qty_entry.pack(side="left", padx=2)
        qty_entry.insert(0, "1")
        
        # Price
        price_entry = ctk.CTkEntry(vals_frame, width=60, height=26, font=ctk.CTkFont(size=11), placeholder_text="Çmimi")
        price_entry.pack(side="left", padx=2)
        price_entry.insert(0, "0")
        
        # Subtotal Label (Auto calc)
        subtotal_label = ctk.CTkLabel(vals_frame, text="0.00 €", font=ctk.CTkFont(size=11, weight="bold"), text_color=self.primary_color)
        subtotal_label.pack(side="right", padx=5)
        
        # Delete btn (top right of card technically, but putting here for now)
        del_btn = ctk.CTkButton(card, text="✕", width=20, height=20, fg_color="transparent", text_color="#E74C3C", hover_color="#FFF0F0", command=lambda r=row_id: self.remove_item_row(r))
        ...
        self.items.append({
            'row': row_id, 'container': card, 
            'desc_entry': desc_entry, 'unit_entry': unit_entry, 
            'qty_entry': qty_entry, 'price_entry': price_entry,
    ...
    def load_offer_data(self):
        ...
        for item in self.offer.items:
            self.add_module_item(); last_item = self.items[-1]
    ...
    def get_offer_data(self):
        ...
        for item in self.items:
            ...
            # If description is empty but numbers are there, we might still want it, or default desc
            qty_val = item['qty_entry'].get()
            if not desc and not qty_val: continue # Skip completely empty ones

            if not desc: desc = "Artikull"
            try:
                qty = float(item['qty_entry'].get().replace(",", "."))
                price = float(item['price_entry'].get().replace(",", "."))
        ...
        if not items and not description:
             messagebox.showerror("Gabim", "Shto artikuj ose përshkrim!"); return None
