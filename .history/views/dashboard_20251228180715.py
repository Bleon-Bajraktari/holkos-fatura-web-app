...
class DashboardView(ctk.CTkFrame):
    ...
    def create_stat_card(self, index, label_text, key, icon, color):
        ...
        # Icon Container
        icon_box = ctk.CTkFrame(card, fg_color=f"{color}15", width=45, height=45, corner_radius=10) # 15% opasitet
    ...
    def render_recent_rows(self, invoices):
        ...
        for i, inv in enumerate(invoices):
            ...
            # Status Badge
            badge_color = "#2ECC71" if status == "PAID" else "#FF6600" if status == "SENT" else "#95A5A6"
            badge = ctk.CTkFrame(row, fg_color=f"{badge_color}15", height=24, corner_radius=6)
            badge.grid(row=0, column=4, padx=15, sticky="w")
            
            ctk.CTkLabel(badge, text=status, font=ctk.CTkFont(family="Inter", size=10, weight="bold"), text_color=badge_color).pack(padx=8)
