"""
Dashboard view - v1.1.2 (Fix për mbivendosjen e shkrimeve)
"""
...
class DashboardView(ctk.CTkFrame):
    ...
    def create_stat_card(self, index, label_text, key, icon, color):
        """Krijon një kartelë - Fix për layout-in e tekstit"""
        card = ctk.CTkFrame(self.stats_container, fg_color="#FFFFFF", height=120, corner_radius=12, border_width=1, border_color=self.border_color)
        card.grid(row=0, column=index, padx=8, pady=5, sticky="ew")
        self.stats_container.grid_columnconfigure(index, weight=1)
        card.pack_propagate(False)
        
        # Përdorim një container të brendshëm me padding për fshirjen e mbivendosjes
        ...
        icon_box.pack(anchor="w", pady=(0, 10))
        icon_box.pack_propagate(False)
        ctk.CTkLabel(icon_box, text=icon, font=ctk.CTkFont(size=18), text_color=color).pack(expand=True)
        
        # Labli (Gri)
        ctk.CTkLabel(inner, text=label_text, font=ctk.CTkFont(family="Inter", size=11), text_color=self.text_secondary, anchor="w").pack(fill="x")
        
        # Shifra (Zezë)
        val_lbl = ctk.CTkLabel(inner, text="...", font=ctk.CTkFont(family="Inter", size=22, weight="bold"), text_color=self.text_main, anchor="w")
        val_lbl.pack(fill="x")
