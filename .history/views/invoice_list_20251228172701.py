...
class InvoiceListView(ctk.CTkFrame):
    ...
    def add_invoice_row(self, parent, inv):
        ...
        ctk.CTkLabel(row_frame, text=total_val, font=ctk.CTkFont(family="Inter", size=13, weight="bold"), text_color=self.text_main).grid(row=0, column=3, sticky="w", padx=10)
        
        # 5. Statusi (Badge)
        status = str(inv.get('status', '')).upper()
        s_colors = {
            "PAID": ("#D4EDDA", "#155724"), # Green
            "SENT": ("#FFF3CD", "#856404"), # Yellow/Gold
            "DRAFT": ("#E2E3E5", "#383D41")  # Gray
        }
        bg, fg = s_colors.get(status, ("#F8F9FA", "#6C757D"))
        
        status_frame = ctk.CTkFrame(row_frame, fg_color=bg, corner_radius=6, height=24)
        status_frame.grid(row=0, column=4, sticky="w", padx=10)
        status_frame.pack_propagate(False)
        ctk.CTkLabel(status_frame, text=status, font=ctk.CTkFont(family="Inter", size=10, weight="bold"), text_color=fg).pack(expand=True, padx=8)
