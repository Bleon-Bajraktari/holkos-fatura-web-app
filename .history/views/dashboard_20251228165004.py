...
class DashboardView(ctk.CTkFrame):
    ...
    def load_statistics(self):
        ...
        try:
            ...
            month_invoices = [inv for inv in invoices if inv['date'] >= first_day]
            ...
            self.stats_labels['month_invoices'].configure(text=str(len(month_invoices)))
            ...
            # Faturat e fundit
            self.load_recent_invoices(invoices[:8])
    ...
    def load_recent_invoices(self, invoices):
        ...
        for i, inv in enumerate(invoices, start=1):
            # Row Frame for hover effect simulation (simplified)
            row_data = [
                inv['invoice_number'],
            ...
            for j, data in enumerate(row_data):
                color = None
                if j == 4: # Status
                    color = "#4CAF50" if str(data).lower() == "paid" else "#FFC107" if str(data).lower() == "sent" else "gray"
