...
class InvoiceFormView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # 2b. Container i Sugjerimeve (Integrim në layout)
        self.suggestions_container = ctk.CTkFrame(self.scrollable_frame, fg_color="transparent")
        # Nuk e paketojmë fillimisht
    ...
    def show_suggestions(self, matches):
        """Shfaq listën e sugjerimeve të integruar në formë"""
        # Pastrimi
        for widget in self.suggestions_container.winfo_children():
            widget.destroy()

        # Paketo container-in nëse nuk është (ndërmjet info dhe artikujve)
        self.suggestions_container.pack(fill="x", padx=20, pady=(0, 10), after=self.scrollable_frame.winfo_children()[1]) # Pas info_frame
        
        # Header i vogël për qartësi
        header_lbl = ctk.CTkLabel(
            self.suggestions_container, 
            text="Kërkimi i klientit:", 
            font=ctk.CTkFont(size=11, weight="bold"), 
            text_color="#666666"
        )
        header_lbl.pack(anchor="w", padx=10, pady=(5, 5))
        
        # Scrollable area për rezultatet
        ...
        for name in matches:
            ...
            item_frame = ctk.CTkFrame(results_frame, fg_color="transparent", height=45, cursor="hand2")
