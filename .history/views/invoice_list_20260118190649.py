...
class InvoiceListView(ctk.CTkFrame):
    ...
    def toggle_single_select(self, inv_id, var):
        ...
        if hasattr(self, 'chk_select_all') and hasattr(self, 'current_invoices_list'):
            total = len(self.current_invoices_list)
    ...
    def bulk_email(self):
        ...
        # Dialog për të caktuar email-in marrës
        dialog = ctk.CTkInputDialog(
            text=f"Do të dërgohen {count} fatura.\n\nShkruani email-in ku dëshironi t'i dërgoni\nose lini bosh për t'ua dërguar klientëve përkatës:", 
            title="Dërgim Faturash"
        )
        # Center the dialog somewhat manually or let it be default
        override_email = dialog.get_input()
        
        # Nëse përdoruesi shtyp Cancel (në disa versione kthen None)
        if override_email is None:
            return

        override_email = override_email.strip()
        target_desc = override_email if override_email else "klientëve përkatës"
        
        if not messagebox.askyesno("Konfirmim", f"A jeni të sigurt se doni të dërgoni {count} fatura te {target_desc}?"):
            return
        ...
        def run_email_process():
            ...
            # Use current list to find emails if no override is provided
            email_map = {inv['id']: inv.get('client_email') for inv in getattr(self, 'current_invoices_list', [])}
            
            # Nëse kemi override (një email specifik), i dërgojmë të gjitha në një email
            if override_email:
                pairs = []
                for inv_id in list(self.selected_ids):
                    try:
                        ...
                        if pdf_path and os.path.exists(pdf_path):
                            pairs.append((inv, pdf_path))
                        else:
                            failed += 1
                            errors.append(f"Fatura {inv.invoice_number}: PDF mungon")
                    except Exception as e:
                        failed += 1
                        errors.append(f"ID {inv_id}: {str(e)}")
                
                if pairs:
                    ok, msg = EmailService.send_bulk_invoices(pairs, override_email)
                    if ok:
                        success = len(pairs)
                    else:
                        failed += len(pairs)
                ...
                for inv_id in list(self.selected_ids):
                    try:
                        ...
                        if not dest_email:
                            failed += 1
                            errors.append(f"ID {inv_id}: Mungon email-i i klientit")
                            continue
                        ...
                        # Dërgo email (is_to_company=False për formatin e klientit)
                        ok, msg = EmailService.send_invoice(inv, dest_email, pdf_path, is_to_company=False)
                        if ok:
                            success += 1
                        else:
                            failed += 1
