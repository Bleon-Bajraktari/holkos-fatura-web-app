...
class InvoiceListView(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        # AKTIVIZOJME SCALING VETËM PËR KËTË KOMPONENT
    ...
    def create_widgets(self):
        # 1. Top Section - Reduktuar me 0.85 për të simuluar zoom
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family="Inter", size=26, weight="bold"), # Size reduktuar nga 32
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family="Inter", size=12), # Size reduktuar nga 14
        ...
        def toggle_click(event=None):
            if content_frame.winfo_manager():
                ...
                if not getattr(content_frame, 'is_populated', False):
                    for inv in invoices: self.add_invoice_row(content_frame, inv)
    ...
    def add_invoice_row(self, parent, inv):
        ...
        for i, w in enumerate(weights): row_frame.grid_columnconfigure(i, weight=w, uniform="col")
        ...
        if pdf_path and not os.path.exists(pdf_path):
            ...
            if os.path.exists(rel_path): actual_path = rel_path
            else: actual_path = None
    ...
    def handle_pdf_action(self, inv_id, path):
        if path and os.path.exists(path):
            ...
            except Exception as e: messagebox.showerror("Gabim", f"Hapja dështoi: {e}")
        else:
            if messagebox.askyesno("PDF", "Ta gjeneroj PDF-në?"): self.generate_pdf(inv_id)
