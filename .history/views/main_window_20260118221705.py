    ...
    def update_status_indicator(self):
        ...
        def do_check():
            try:
                ...
                is_offline = Database._is_offline
                # Përditëso UI në main thread
                if self.winfo_exists():
                    self.after(0, lambda: self._apply_status(is_offline))
            except Exception as e:
                print(f"Status check error: {e}")
            finally:
                self._checking_status = False
                # Cakto kontrollin e radhës
                if self.winfo_exists():
                    self.after(3000, self.update_status_indicator)
