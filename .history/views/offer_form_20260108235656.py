...
class OfferFormView(ctk.CTkFrame):
    ...
    def add_aligned_row(self, initial_data=None, row_type='item'):
        ...
        if row_type == 'header':
            ...
            if row_type == 'item':
                # Right Side: Calculations Container
                right_side_container = ctk.CTkFrame(row_frame, fg_color="transparent")
                right_side_container.grid(row=1, column=1, columnspan=2, sticky="ew", padx=10, pady=(0, 10))
                
                # Label
                ctk.CTkLabel(right_side_container, text="Kalkulimi:", font=ctk.CTkFont(size=10, weight="bold"), text_color=self.text_secondary).pack(side="left", padx=(0, 10))
                ...
                # Result Part
                ctk.CTkLabel(right_side_container, text="=", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.text_secondary).pack(side="left", padx=5)
                
                subtotal_label = ctk.CTkLabel(right_side_container, text="0.00 €", font=ctk.CTkFont(size=13, weight="bold"), text_color=self.primary_color)
                subtotal_label.pack(side="left", padx=5)
                
                # Add Module Button (Inline)
                add_mod_btn = ctk.CTkButton(right_side_container, text="+ Shto vlerë", width=70, height=24, font=ctk.CTkFont(size=11), fg_color="#F1F3F5", text_color=self.text_main, hover_color="#E9ECEF", corner_radius=12, command=lambda: self.add_module_to_row(row_id))
    ...
    def add_module_to_row(self, row_id, label_text="", unit_text=""):
        ...
        if len(item['modules']) > 0:
            ctk.CTkLabel(mod_frame, text="x", font=ctk.CTkFont(size=12, weight="bold"), text_color=self.text_secondary).pack(side="left", padx=4)

        # Styled Input Group
        input_container = ctk.CTkFrame(mod_frame, fg_color="#F8F9FA", corner_radius=6, border_width=1, border_color="#DEE2E6")
        input_container.pack(side="left")
        
        # Value
        val_entry = ctk.CTkEntry(input_container, width=55, height=24, font=ctk.CTkFont(size=11), fg_color="transparent", border_width=0, placeholder_text="0")
        val_entry.pack(side="left", padx=(2, 0))
        
        # Unit
        unit_entry = ctk.CTkEntry(input_container, width=35, height=24, font=ctk.CTkFont(size=10, weight="bold"), fg_color="transparent", border_width=0, text_color=self.primary_color, placeholder_text="nj.")
