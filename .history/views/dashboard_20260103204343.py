...
class DashboardView(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Merret muaji aktual për titullin
        current_month = datetime.now().strftime("%B") # Ose përkthim në shqip
        MONTHS_AL = {"January": "Janar", "February": "Shkurt", "March": "Mars", "April": "Prill", "May": "Maj", "June": "Qershor",
                     "July": "Korrik", "August": "Gusht", "September": "Shtator", "October": "Tetor", "November": "Nëntor", "December": "Dhjetor"}
        curr_month_al = MONTHS_AL.get(current_month, current_month)
        
        self.stats_labels = {}
        stats_info = [
            (f"Ardhura ({curr_month_al})", "month_total", "📅", "#3498DB"),
            (f"Ardhura ({datetime.now().year})", "year_total", "💰", "#2ECC71"),
            ("Faturat e Papaguara", "unpaid_total", "⚠️", "#E74C3C"),
            ("Numri Total", "total_invoices", "📄", "#9B59B6")
        ]
    ...
    def create_stat_card(self, index, label_text, key, icon, color):
        ...
        light_colors = {"#3498DB": "#EBF5FB", "#9B59B6": "#F5EEF8", "#2ECC71": "#EAFAF1", "#E67E22": "#FEF5E7", "#E74C3C": "#FDE2E2"}
    ...
    def load_statistics(self):
        try:
            stats = Invoice.get_dashboard_stats(self.db)
            
            # Formato vlerat
            month_total = float(stats.get('month_total', 0) or 0)
            year_total = float(stats.get('year_total', 0) or 0)
            unpaid_total = float(stats.get('unpaid_total', 0) or 0)
            total_invoices = stats.get('total_invoices', 0)
            
            self.stats_labels['month_total'].configure(text=f"{month_total:,.2f} €".replace(",", " "))
            self.stats_labels['year_total'].configure(text=f"{year_total:,.2f} €".replace(",", " "))
            self.stats_labels['unpaid_total'].configure(text=f"{unpaid_total:,.2f} €".replace(",", " "))
