        ...
        if offer.items:
            ...
            for item in offer.items:
                ...
                if row_type == 'header':
                    ...
                    h_style = ParagraphStyle("OfferHeader", parent=self.normal_style, fontSize=14, fontName='Helvetica-Bold', textColor=colors.black)
                    story.append(Paragraph(item['description'].upper(), h_style))
                ...
                if desc_text:
                    ...
                    for line in lines:
                        if line.strip():
                            style = self.note_style if line.strip().startswith("*") else self.normal_style
                # COMPACT STYLE: Description = Formula
                # Build the text segment: "Description ... = 1000 € m2"
                ...
                if custom_attr and isinstance(custom_attr, list):
                    # Check if at least one segment has a numeric value
                    ...
                    if has_any_val:
                        for idx, attr in enumerate(custom_attr):
                            ...
                            unit = str(attr.get('unit', '')).strip().replace("m2", "m²").replace("m3", "m³")
                            
                            if val:
                                # We only put 'x' if there was a PREVIOUS segment that also had a VALUE
                                has_previous_val = any(s.startswith("<b>") for s in calc_segments)
                                if has_previous_val:
                                    calc_segments.append("<font color='#666666'>x</font>")
                                calc_segments.append(f"<b>{val}</b> {unit}")
                            elif unit:
                                # If no value, just append the unit (suffix)
                                calc_segments.append(unit)
                
                formula_html = " ".join(calc_segments).strip() if has_any_val else ""
                
                # Combine into a single paragraph or a very tight table
                if desc_text and formula_html:
                    # Full line: Description = Formula
                    full_line = f"{desc_text} <font color='#666666'>=</font> {formula_html}"
                    style = self.note_style if desc_text.startswith("*") else self.normal_style
                    story.append(Paragraph(full_line, style))
                    ...
                    # Only Formula
                    story.append(Paragraph(f"<font color='#666666'>=</font> {formula_html}", self.right_style))
                    story.append(Spacer(1, 2*mm))
