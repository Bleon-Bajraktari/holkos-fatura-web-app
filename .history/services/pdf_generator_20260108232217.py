...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        # ITEM ROWS (Optional, if they exist)
        # Rendered cleanly: Description (Left) ...... Qty Unit x Price = Total (Right)
        if offer.items:
            story.append(Spacer(1, 5*mm))
            ...
            for item in offer.items:
                ...
                # Format: "10.00 m2   x   15.00 €   =   150.00 €"
                qty = self.format_number(item['quantity'])
                unit = item['unit']
                price = self.format_number(item['unit_price']) + " €"
                total = self.format_number(item['subtotal']) + " €"
                
                # Using a 2-column table: Description (Left), Calc (Right)
                right_text = f"{qty} {unit}  x  {price}  =  {total}"
                
                items_data.append([
                    Paragraph(desc, self.normal_style),
                    Paragraph(right_text, self.right_style)
                ])
                
            list_table = Table(items_data, colWidths=[100*mm, 90*mm])
