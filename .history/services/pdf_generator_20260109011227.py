...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None, manual_font_size=None):
        ...
        company_table.setStyle(TableStyle([
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("ALIGN", (0, 0), (-1, -1), "LEFT"),
            ("LEFTPADDING", (0, 0), (-1, -1), 0), # Force 0 padding
        ...
        if offer.subject:
            # Stili i titullit: E Zeze, Bold, sic shkruar
            ...
            # Wrap in Table with 0 padding to match other tables alignment perfectly
            title_para = Paragraph(offer.subject, offer_title_style)
            title_table = Table([[title_para]], colWidths=[190*mm])
            ...
            story.append(Paragraph("OFERTË", self.bold_style)) 
        
        story.append(Spacer(1, 10*mm))
        
        if offer.items:
            # DYNAMIC SCALING: Detect content density
            content_sample = " ".join([i.get('description','') for i in offer.items])
            total_len = len(content_sample)
            count = len(offer.items)
            ...
            # Specialized Styles
            r_style = ParagraphStyle("RNormal", parent=self.normal_style, fontSize=base_font, leading=leading)
            r_note = ParagraphStyle("RNote", parent=self.note_style, fontSize=base_font, leading=leading)
            r_header = ParagraphStyle("RHead", parent=self.normal_style, fontSize=base_font+3, leading=leading+4, fontName="Helvetica-Bold", textColor=colors.black)
            r_calc = ParagraphStyle("RCalc", parent=self.right_style, fontSize=base_font, leading=leading)
            ...
            for item in offer.items:
                ...
                if row_type == 'header':
                    ...
                    story.append(Paragraph(item['description'].upper(), r_header))
                    story.append(Spacer(1, 4*mm))
                    continue
                
                if row_type == 'text':
                    lines = item.get('description', '').split('\n')
                    for line in lines:
                        if line.strip():
                            story.append(Paragraph(line, r_note if line.strip().startswith("*") else r_style))
                    story.append(Spacer(1, 6*mm))
                    continue

                # ITEM ROW [Desc ON LEFT | Calc ON RIGHT]
                desc_text = item.get('description', '').strip()
                calc_segments = []
                custom_attr = item.get('custom_attributes', [])
                has_any_val = False
                
                if isinstance(custom_attr, list):
                    has_any_val = any(str(attr.get('value', '')).strip() for attr in custom_attr)
                    if has_any_val:
                        for idx, attr in enumerate(custom_attr):
                ...
                if desc_text and formula_html:
                    # WE USE A TABLE TO FORCE ALIGNMENT: Desc (Left) --- Formula (Right)
                    data = [[Paragraph(desc_text, r_style), Paragraph(f"<font color='#666666'>=</font> {formula_html}", r_calc)]]
                    # colWidths: 130mm for text, 55mm for calculation (Standard A4 is ~190mm usable width)
                    item_table = Table(data, colWidths=[130*mm, 60*mm])
                    ...
                    for line in desc_text.split('\n'):
                        if line.strip(): story.append(Paragraph(line, r_style))
                elif formula_html:
                    story.append(Paragraph(f"<font color='#666666'>=</font> {formula_html}", r_calc))

                story.append(Spacer(1, 6*mm))

            # TOTAL REMOVED AS PER USER REQUEST
