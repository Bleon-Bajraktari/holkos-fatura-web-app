...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        # -------------------------------
        # TITULLI "OFERTË NR. XX/VITI"
        # -------------------------------
        year = offer.date.year
        offer_num = offer.offer_number
        if "OF-" in offer_num:
            offer_num = offer_num.replace("OF-", "")
            
        title_text = f"OFERTË {offer_num}"
        title = Paragraph(title_text, self.title_style)
        story.append(title)
        
        story.append(Spacer(1, 6*mm))
        date_text = f"Data: {self.format_date(offer.date)}"
        subtitle = Paragraph(date_text, self.subtitle_style)
        story.append(subtitle)
        ...
        # -------------------------------
        # HEADER (Kompania & Blerësi) - Reused Logic
        # -------------------------------
        logo_path = company.logo_path if company.logo_path and os.path.exists(company.logo_path) else None
        if not logo_path:
            logo_path = os.path.join(IMAGES_DIR, "logo.png")
        ...
        # Logo Logic (Copied from generate for consistency)
        if os.path.exists(logo_path):
            try:
                pil_img = PILImage.open(logo_path)
        ...
        # -------------------------------
        # ARTIKUJT (Me kolone Unit)
        # -------------------------------
        items = [
            ["PERSHKRIMI", "NJESIA", "SASIA", "CMIMI", "NENTOTALI"]
        ]
        
        for item in offer.items:
            items.append([
                Paragraph(item['description'], self.normal_style), # Paragraph allow wrap
                item['unit'],
                self.format_number(item['quantity']),
                self.format_number(item['unit_price'], with_currency=True),
                self.format_number(item['subtotal'], with_currency=True)
            ])
            
        while len(items) < 6:
            items.append(["", "", "", "", ""])
            
        # [Desc, Unit, Qty, Price, Subtotal] -> Total 185mm
        items_table = Table(items, colWidths=[75*mm, 20*mm, 25*mm, 30*mm, 35*mm])
        items_table.setStyle(TableStyle([
            ("GRID", (0, 0), (-1, -1), 1, colors.black),
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTSIZE", (0, 0), (-1, -1), 10),
            ("ALIGN", (2, 1), (-1, -1), "RIGHT"), # Qty, Price, Subtotal right align
        ...
        # -------------------------------
        totals = [
            ["", "TOTAL PA TVSH", self.format_number(offer.subtotal, with_currency=True)],
            ["", "TVSH %", f"{float(offer.vat_percentage):.2f}%"],
            ["", "TVSH SHUMA", self.format_number(offer.vat_amount, with_currency=True)],
            ["", "TOTALI Ofertës", self.format_number(offer.total, with_currency=True)],
        ]
        
        totals_table = Table(totals, colWidths=[100*mm, 45*mm, 40*mm])
        totals_table.setStyle(TableStyle([
            ("GRID", (2, 0), (2, -1), 1, colors.black),
            ("ALIGN", (1, 0), (2, -1), "RIGHT"),
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
            ("FONTNAME", (1, 3), (2, 3), "Helvetica-Bold"), # Bold Total Row
            ("FONTSIZE", (2, 3), (2, 3), 12),
            ("LEFTPADDING", (0, 0), (-1, -1), 6),
            ("RIGHTPADDING", (0, 0), (-1, -1), 6),
