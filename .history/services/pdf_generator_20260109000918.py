            ...
            for item in offer.items:
                ...
                if custom_attr and isinstance(custom_attr, list):
                    for idx, attr in enumerate(custom_attr):
                        ...
                        if val:
                            if idx > 0: calc_segments.append("<font color='#666666'>x</font>")
                            # Professional unit formatting (if m2/m3)
                            u_fmt = unit.replace("m2", "m²").replace("m3", "m³")
                            calc_segments.append(f"<b>{val}</b> {u_fmt}")
                    
                    sub = float(item['subtotal'])
                    if sub > 0:
                        if len(calc_segments) > 1:
                            # Complex formula
                            formula_text = " ".join(calc_segments) + f" = <b>{self.format_number(sub)} €</b>"
                        else:
                            # Single value (like a fixed price announcement)
                            formula_text = f"<b>{calc_segments[0]}</b> = <b>{self.format_number(sub)} €</b>" if "=" not in calc_segments[0] else calc_segments[0]
                        
                        formula_style = ParagraphStyle("CalcRight", parent=self.right_style, fontSize=11, leading=14)
                        right_flowables.append(Paragraph(formula_text, formula_style))
                else:
                    # Fallback (Standard Qty/Price)
                    qty_val = float(item['quantity'])
                    price_val = float(item['unit_price'])
                    if qty_val > 0 or price_val > 0:
                        qty = self.format_number(qty_val)
                        unit = item['unit'].replace("m2", "m²").replace("m3", "m³")
                        price = self.format_number(price_val)
                        total = self.format_number(item['subtotal'])
                        calc_text = f"<b>{qty} {unit}</b> x <b>{price} €</b> = <b>{total} €</b>"
                        right_flowables.append(Paragraph(calc_text, ParagraphStyle("CalcRightStd", parent=self.right_style, fontSize=11)))

                # Item Table Row [ 70% | 30% ]
                if left_flowables or right_flowables:
                    item_table = Table([[left_flowables, right_flowables]], colWidths=[125*mm, 65*mm])
