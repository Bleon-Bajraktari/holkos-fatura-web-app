    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        if offer.items:
            ...
            for item in offer.items:
                ...
                # Identify bullets or normal text
                left_flowables = []
                ...
                # If quantity/price are 0, maybe hide calculation part? Or show 0?
                # User asked for "Calculation part". If it's pure text row, right side is empty?
                # Check if Qty or Price > 0
                ...
                if qty_val > 0 or price_val > 0:
                    ...
                    right_flowables.append(Paragraph("", self.normal_style))

                main_table_data.append([left_flowables, right_flowables])
            
            # Build the Table
            # Page width ~190mm. Left ~65%, Right ~35%
            t = Table(main_table_data, colWidths=[115*mm, 75*mm])
            ...
            # TOTAL at the very bottom right
            story.append(Spacer(1, 5*mm))
            total_text = f"TOTALI: {self.format_number(offer.total, with_currency=True)}"
            story.append(Paragraph(f"<b>{total_text}</b>", ParagraphStyle("TotalRightBox", parent=self.right_style, fontSize=12, textColor=colors.HexColor("#356CA3"))))
            story.append(Spacer(1, 10*mm))
