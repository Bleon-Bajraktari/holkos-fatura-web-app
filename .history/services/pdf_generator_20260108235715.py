            ...
            for item in offer.items:
                ...
                if custom_attr and isinstance(custom_attr, list):
                    for idx, attr in enumerate(custom_attr):
                        ...
                        unit = attr.get('unit', '')
                        if val:
                            if idx > 0: calc_segments.append("x")
                            calc_segments.append(f"<b>{val} {unit}</b>")
                    
                    sub = float(item['subtotal'])
                    if sub > 0:
                        calc_text = " ".join(calc_segments) + f" = <b>{self.format_number(sub)} €</b>"
                        right_flowables.append(Paragraph(calc_text, ParagraphStyle("CalcRightFormula", parent=self.right_style, fontSize=11, textColor=colors.HexColor("#FF6600"))))
                else:
                    # Fallback
