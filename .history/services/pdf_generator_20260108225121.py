...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        # Informatat shtese poshte titullit
        info_style = ParagraphStyle("InfoStyle", fontSize=11, leading=14)
        story.append(Paragraph(f"Kompania: {company.name}", info_style))
        story.append(Paragraph(f"Klienti: {client.name}", info_style))
        story.append(Paragraph(f"Data: {self.format_date(offer.date)}", info_style))
        
        story.append(Spacer(1, 10*mm))
        
        # Subheader "PUNIMET E PËRFSHIRA"
        subheader_style = ParagraphStyle(
            "SubHeader",
            fontSize=12,
            textColor=colors.HexColor("#356CA3"),
            fontName="Helvetica-Bold",
            spaceAfter=10,
            spaceBefore=10
        )
        ...
        for item in offer.items:
            ...
            items_data.append([
                ...
                Paragraph(qty_unit, self.right_style), # Qty
                Paragraph(f"x {price_calc}", self.right_style), # Price
                Paragraph(total_line, self.right_style) # Total
            ])
            
        list_table = Table(items_data, colWidths=[90*mm, 35*mm, 30*mm, 35*mm])
