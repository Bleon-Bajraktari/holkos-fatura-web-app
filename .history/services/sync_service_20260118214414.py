            ...
            for table in tables:
                ...
                for year in sorted(by_year.keys()):
                    ...
                    for item in year_items:
                        ...
                        sql = f"REPLACE INTO {table} ({', '.join(columns)}) VALUES ({', '.join(['%s']*len(columns))})"
                        with db.connection.cursor() as cc: cc.execute(sql, tuple(item[c] for c in columns))
                        with db.backup_connection.cursor() as lc: lc.execute(sql, tuple(item[c] for c in columns))
