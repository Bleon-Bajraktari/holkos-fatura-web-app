...
class PDFGenerator:
    ...
    def __init__(self):
        ...
        self.style_right = ParagraphStyle('RightAlign', parent=self.styles['Normal'], alignment=TA_RIGHT)
        self.setup_styles()
    ...
    def fit_text(self, text, max_chars=11, base_size=10):
        """Kthen tekstin si Paragraph me font të zvogëluar nëse është i gjatë"""
        if len(text) > max_chars:
            # Llogarit madhësinë e re (proporcionale)
            # Nëse 11 chars -> 10pt. Nëse 22 chars -> 5pt.
            # Formula empirike: new_size = base_size * (max_chars / len)
            new_size = int(base_size * (max_chars / len(text)))
            if new_size < 6: new_size = 6 # Minimumi
            return Paragraph(f'<font size="{new_size}">{text}</font>', self.style_right)
        return text
    ...
    def generate(self, invoice: Invoice, output_path=None):
        ...
        for item in invoice.items:
            qty_str = f"{float(item['quantity']):,.2f}".replace(",", " ").replace(".", ",")
            price_str = self.format_number(item['unit_price'], with_currency=True)
            subtotal_str = self.format_number(item['subtotal'], with_currency=True)
            
            # Përdor fit_text për çmimin dhe nëntotalin
            items.append([
                item['description'], 
                qty_str, 
                self.fit_text(price_str, max_chars=10, base_size=10), 
                self.fit_text(subtotal_str, max_chars=10, base_size=10)
            ])
        ...
        # Tabela me gjerësi origjinale (Përshkrimi i gjerë)
        ...
        items_table.setStyle(TableStyle([
            ...
            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTSIZE", (0, 0), (-1, 0), 11), # Header font normal
            ("FONTSIZE", (0, 1), (-1, -1), 10.5), # Content font normal
            ("ALIGN", (1, 1), (-1, -1), "RIGHT"),
        ...
        totals = [
            [legal_paragraph, "TOTAL PA TVSH", self.fit_text(self.format_number(invoice.subtotal, with_currency=True), 12, 11)],
            ["", "TVSH %", f"{float(invoice.vat_percentage):.2f}%"],
            ["", "TVSH SHUMA", self.fit_text(self.format_number(invoice.vat_amount, with_currency=True), 12, 11)],
            ["", "PER PAGESE", self.fit_text(self.format_number(invoice.total, with_currency=True), 10, 14)], # Max chars më pak numra të mëdhenj këtu sepse font është i madh
        ]
        
        # Gjerësitë origjinale: [Shënimi(L), Etiketa(R), Shuma(R)]
        totals_table = Table(totals, colWidths=[110*mm, 50*mm, 25*mm])
