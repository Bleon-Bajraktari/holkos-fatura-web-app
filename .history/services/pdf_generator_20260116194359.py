    ...
    def generate_offer(self, offer: Offer, output_path=None, manual_font_size=None):
        ...
        if offer.items:
            ...
            for item in offer.items:
                ...
                if row_type == 'header':
                    desc_text = item.get('description', '').strip().upper()
                    if desc_text:
                        # Header Table spanning full width
                        ...
                        if has_border:
                            style_cmds.extend([
                                ...
                                ('TOPPADDING', (0,0), (-1,-1), 6),
                                ('BOTTOMPADDING', (0,0), (-1,-1), 6),
                                ('BACKGROUND', (0,0), (-1,-1), colors.Color(0.97, 0.97, 0.97)) # Light bg for header box
                            ])
                    ...
                    if paras:
                        # Text Table spanning full width
                        data = [[paras]] # List of paragraphs in one cell
                        t = Table(data, colWidths=[190*mm])
                        style_cmds = [
                            ('VALIGN', (0,0), (-1,-1), 'TOP'),
                            ('LEFTPADDING', (0,0), (-1,-1), 0),
            ...
            for i, item in enumerate(render_items):
                ...
                if next_item and item['has_border'] and next_item['has_border']:
                    # Both have borders: 'Glued' together (No Spacer)
                    # We might want a tiny negative spacer if lines look doubled, but 0 usually works for "touching"
                    pass 
                else:
                    # Default spacing
                    story.append(Spacer(1, 6*mm))
