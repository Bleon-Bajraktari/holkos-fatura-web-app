...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        # Main Content Table: [Description | Items]
        # We will create two lists of Flowables: one for left, one for right
        left_content = []
        right_content = []
        ...
        if offer.description:
            ...
            for line in desc_lines:
                if line.strip():
                    ...
                    left_content.append(Paragraph(line, style))
                    left_content.append(Spacer(1, 2*mm))
        
        # RIGHT: Items Modules
        if offer.items:
            # Header for items part
            right_content.append(Paragraph("<b> </b>", self.normal_style)) # Spacer/aligner
            
            for item in offer.items:
                # Item Block
                # Description (Bold?)
                desc = item['description']
                right_content.append(Paragraph(f"<b>{desc}</b>", self.normal_style))
                
                # Calc line: 10 m2 x 5.00 € = 50.00 €
                qty = self.format_number(item['quantity'])
                unit = item['unit']
                price = self.format_number(item['unit_price'])
                total = self.format_number(item['subtotal'])
                
                calc_text = f"{qty} {unit} x {price} € = <b>{total} €</b>"
                right_content.append(Paragraph(calc_text, ParagraphStyle("CalcRight", parent=self.right_style, fontSize=10, textColor=colors.HexColor("#555555"))))
                
                right_content.append(Spacer(1, 3*mm))
            
            # TOTAL at bottom of right column
            right_content.append(Spacer(1, 5*mm))
            total_text = f"TOTALI: {self.format_number(offer.total, with_currency=True)}"
            right_content.append(Paragraph(f"<b>{total_text}</b>", ParagraphStyle("TotalRightBox", parent=self.right_style, fontSize=12, textColor=colors.HexColor("#356CA3"))))
        
        # Construct the Main Table
        # Col widths: Left ~60-65%, Right ~35-40%
        # Page width ~190mm avail
        layout_table = Table(
        ...
        layout_table.setStyle(TableStyle([
            ...
            # Optional: Vertical line divider?
