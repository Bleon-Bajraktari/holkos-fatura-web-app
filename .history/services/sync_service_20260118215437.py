...
class SyncService:
    ...
    def sync_local_to_cloud():
        ...
        try:
            ...
            # 2. SINKRONIZO FATURAT DHE OFERTAT (ME RINUMËRIM)
            for t in ['invoices', 'offers']:
                with db.connection.cursor() as c_cur:
                    c_cur.execute(f"SELECT * FROM {t}")
                    cloud_data = {row['id']: row for row in c_cur.fetchall()}
                with db.backup_connection.cursor() as l_cur:
                    l_cur.execute(f"SELECT * FROM {t}")
                    local_data = {row['id']: row for row in l_cur.fetchall()}
                
                all_docs = {**cloud_data, **local_data}
                if not all_docs: continue

                # Rendit sipas kohës
                sorted_docs = sorted(all_docs.values(), key=lambda x: x['created_at'])
                by_year = {}
