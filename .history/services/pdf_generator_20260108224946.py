...
class PDFGenerator:
    ...
    def generate_offer(self, offer: Offer, output_path=None):
        ...
        # TITULLI "OFERTË PËR..."
        # -------------------------------
        # Spacer për të ndarë nga header
        story.append(Spacer(1, 10*mm))

        # Titulli i Subjektit (Blu dhe Bold, si në foto)
        # Nese kemi subject, shkruajme "OFERTË PËR [SUBJECT]"
        # Nese jo, vetem "OFERTË PËR PUNIME"
        subject_text = f"OFERTË PËR {offer.subject.upper()}" if offer.subject else "OFERTË PËR PUNIME"
        
        # Stili i ri për titullin e ofertës
        offer_title_style = ParagraphStyle(
            "OfferTitle",
            parent=self.styles['Heading1'],
            fontSize=14,
            textColor=colors.HexColor("#356CA3"), # Blu e ngjashme me foto
            fontName="Helvetica-Bold",
            spaceAfter=15
        )
        
        story.append(Paragraph(subject_text, offer_title_style))
        
        # Detajet Kompania/Klienti/Data në tekst të thjeshtë poshtë titullit (si në foto)
        # Por useri tha "header ... duhet te merret nga faturat aktuale"
        # Keshtu qe header lart mbetet. 
        # Ne foto ka tekst poshte titullit: "Kompania: ..., Klienti: ..., Data: ..."
        # Meqe header lart i ka keto info, ndoshta i dublikojme per te match-ur stilin e fotos?
        # User tha "vetem pjesa e body... header nga faturat".
        # Body ne foto fillon me Titullin.
        
        # Shto Datën poshtë titullit nëse duhet
        # date_p = Paragraph(f"Data: {self.format_date(offer.date)}", self.normal_style)
        # story.append(date_p)
        # story.append(Spacer(1, 10*mm))

        # -------------------------------
        # PUNIMET E PËRFSHIRA (Sub-header)
        # -------------------------------
        subheader_style = ParagraphStyle(
            "SubHeader",
            fontSize=12,
            textColor=colors.HexColor("#356CA3"), # Blu
            fontName="Helvetica-Bold",
            spaceAfter=10,
            spaceBefore=10
        )
        story.append(Paragraph("PUNIMET E PËRFSHIRA", subheader_style))
        
        # -------------------------------
        # LISTA E ARTIKUJVE (Si liste teksti, jo tabele gri)
        # -------------------------------
        # Formati ne foto: "- Pershkrimi = Cmimi Unit"
        # Ose "- Pershkrimi ... (detaje)"
        ...
        item_style = ParagraphStyle(
            ...
            leading=14,
            leftIndent=10,
            firstLineIndent=-10 # Per bullet effect
        )
        
        for item in offer.items:
            # Nderto tekstin e rreshtit
            # Shembull: "- Lloji i montimit: Me shin te plote = 190€ m²"
            # Nese sasia eshte 1, trego vetem cmimin unitar?
            # User ka formen me Sasi.
            # Do mundohem te kombinoj.
            # "- [Pershkrimi] : [Sasia] [Njesia] x [Cmimi] = [Totali]"
            # Ose nese user do stilin e fotos fiks:
            # "- [Pershkrimi] = [Cmimi]€ [Njesia]" (Nese do vetem kuotim per njesi)
            
            # Mendoj se user do dicka te lexueshme "si text".
            # Do perdorim formatin: "- Description = Unit_Price€ / Unit" (dhe totalin nese sasia > 0)
            
            # Per te qene safe dhe transparent:
            # "- Pershkrimi ... Sasia: 100 m2 ... Cmimi: 45€ ... Totali: 4500€"
            # Por useri do "si ne foto". Fotoja eshte minimaliste.
            
            # Le të bëjmë një format hibrid të pastër:
            # "- Pershkrimi ...................... 100 m² x 45.00 € = 4,500.00 €"
            # Përdorim Table transparente për alignment perfekt
            
            desc = f"- {item['description']}"
            qty_unit = f"{self.format_number(item['quantity'])} {item['unit']}"
            price_calc = f"{self.format_number(item['unit_price'])} €"
            total_line = f"= {self.format_number(item['subtotal'])} €"
            
            # Ne foto duket kështu: "Lloji i montimit: ... = 190€ m2"
            # Kjo do te thote {Description} = {Price} {Unit}
            
            # Nese useri ka sasi > 1, duhet patjeter te tregojme totalin diku.
            # Do bejme nje tabele PA VIJA (transparente)
            
            items_data.append([
                Paragraph(desc, self.normal_style),
                Paragraph(qty_unit, self.right_style),
                Paragraph(f"x {price_calc}", self.right_style),
                Paragraph(total_line, self.right_style)
            ])
            
        # Tabela e listës (transparente)
        # Col widths: Desc (wide), QtyUnit, Price, Total
        list_table = Table(items_data, colWidths=[95*mm, 35*mm, 30*mm, 30*mm])
        list_table.setStyle(TableStyle([
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
            ("LEFTPADDING", (0, 0), (-1, -1), 0),
            ("RIGHTPADDING", (0, 0), (-1, -1), 0),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 6), # Hapesire midis rreshtave
        ]))
        ...
        # -------------------------------
        # TOTALI (Opsional, nese duhet)
        # -------------------------------
        # Ne foto nuk shihet totali, por eshte oferte, zakonisht ka.
        # Do e leme totalin por me stil minimalist poshte, pa tabele te rende
        
        total_text = f"TOTALI: {self.format_number(offer.total, with_currency=True)}"
        total_p = Paragraph(f"<b>{total_text}</b>", ParagraphStyle("TotalRight", parent=self.normal_style, alignment=TA_RIGHT, fontSize=12))
        story.append(total_p)
        story.append(Spacer(1, 10*mm))
        
        # -------------------------------
        # DESCRIPTION (Flexible Text / Notes)
        # -------------------------------
        # "*Ne oferte perfshihet..."
        if offer.description:
            desc_lines = offer.description.split('\n')
            for line in desc_lines:
                if line.strip():
                    # Check nese fillon me * per ta bere italic ose bold
